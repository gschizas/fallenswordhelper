...
const injectBioWidgets = () => { runDefault(import('./bioWidgets-ced474c0.js')); };
const injectGuild = () => { runDefault(import('./guild-8ef07142.js')); };
const injectProfile = () => { runDefault(import('./profile-42657d3d.js').then(function (n) { return n.p; })); };
const injectProfileDropItems = () => { runDefault(import('./injectProfileDropItems-c7711d53.js')); };
const injectQuestBookFull = () => { runDefault(import('./injectQuestBookFull-bb6139cb.js')); };
const inventing = () => { runDefault(import('./inventing-bf4ce935.js')); };
const news = () => { runDefault(import('./news-561c6502.js')); };
const ufsgAllowBack = () => { runDefault(import('./ufsgAllowBack-eb3694ef.js')); };
const viewArchive = () => { runDefault(import('./viewArchive-3fa44882.js')); };

const injectBuffLog = (i) => { import('./injectBuffLog-dc3a8532.js').then((m) => m.default(i)); };
const injectFsBoxContent = (i) => { import('./injectFsBoxContent-07c8d74f.js').then((m) => m.default(i)); };
const injectMonsterLog = (i) => { import('./monstorLog-18e7f34e.js').then((m) => m.default(i)); };
const injectNotepadShowLogs = (i) => { import('./combatLog-92c38a53.js').then((m) => m.default(i)); };
const injectOnlinePlayers = (i) => { import('./injectOnlinePlayers-7d054533.js').then((m) => m.default(i)); };
const injectRecipeManager = (i) => { import('./recipeMgr-0ebcd374.js').then((m) => m.default(i)); };
const insertQuickExtract = (i) => { import('./quickExtract-076b2f73.js').then((m) => m.default(i)); };
const insertQuickWear = (i) => { import('./quickWear-2ef3788a.js').then((m) => m.default(i)); };

const injectAuctionSearch = (i) => { import('./lists-00cbb6e8.js').then((m) => m.injectAuctionSearch(i)); };
const injectQuickLinkManager = (i) => { import('./lists-00cbb6e8.js').then((m) => m.injectQuickLinkManager(i)); };
const injectFindBuffs = (i) => { import('./findBuffs-126c307e.js').then((m) => m.injectFindBuffs(i)); };
const injectFindOther = (i) => { import('./findBuffs-126c307e.js').then((m) => m.injectFindOther(i)); };
...
function getLinkConfig(theNav, myNav) {
  ...
  if (linkConfig.some(([, b]) => b)) {
    import('./injectItems-d966764d.js')
...
function moveUp(title) {
  import('./moveRHSBoxUpOnRHS-844ff6b2.js').then((m) => m.default(title));
}

function moveLeft(title) {
  import('./moveRHSBoxToLHS-d7a401d9.js').then((m) => m.default(title));
}
...
function callAllyEnemy() {
  if (calf.enableAllyOnlineList
    ...
    runDefault(import('./allyEnemy-35aaefe0.js'));
  }
}
...
function callBounties() {
  if (calf.enableWantedList
    ...
    runDefault(import('./activeWantedBounties-54c24d3f.js'));
  }
}

function callGuildInfo() {
  if (calf.enableGuildInfoWidgets) {
    runDefault(import('./addGuildInfoWidgets-30c809b0.js'));
  }
}

function callAllies() {
  if (calf.enableOnlineAlliesWidgets) {
    runDefault(import('./addOnlineAlliesWidgets-5236eb5f.js'));
  }
}

function callTemple() {
  if (calf.enableTempleAlert) {
    runDefault(import('./injectTempleAlert-e7aa44a1.js'));
  }
}

function callUpgrade() {
  if (calf.enableUpgradeAlert) {
    runDefault(import('./injectUpgradeAlert-48bc4a28.js'));
  }
}

function callComposing() {
  if (calf.enableComposingAlert) {
    runDefault(import('./injectComposeAlert-bccc14a2.js'));
  }
}

function statbar() {
  if (getValue('statBarLinks')) {
    runDefault(import('./statBar-92fdf2e3.js'));
  }
}

function staminaCalc() {
  if (getValue('staminaCalculator')) {
    runDefault(import('./injectStaminaCalculator-a7b7a469.js'));
  }
}

function levelCalc() {
  if (getValue('levelUpCalculator')) {
    runDefault(import('./injectLevelupCalculator-9a6bc6fb.js'));
  }
}

function fsBoxLog() {
  if (getValue('fsboxlog')) {
    runDefault(import('./injectFSBoxLog-180e0072.js'));
  }
}

function expandQb() {
  if (getValue('resizeQuickBuff')) {
    runDefault(import('./interceptQuickBuff-8221effe.js'));
  }
}

function joinAll() {
  if (getValue('joinAllLink')) {
    runDefault(import('./injectJoinAllLink-2e6c0b6c.js'));
  }
}

function guildLogHref() {
  if (getValue('useNewGuildLog')) {
    runDefault(import('./changeGuildLogHREF-fb0c6c70.js'));
  }
}

function gameStats() {
  if (getValue('addServerNode')) {
    runDefault(import('./injectServerNode-bef93a04.js'));
  }
}

function scoutTower() {
  if (getValue('addScoutTowerLink')) {
    runDefault(import('./scoutTowerLink-2e0180a7.js'));
  }
}

function guildActivityTracker() {
  if (jQueryPresent() && getValue(defEnableGuildActivityTracker)) {
    runDefault(import('./guildActivity-4b153c43.js'));
  }
}

function seTracker() {
  if (jQueryPresent() && calf.enableSeTracker && calf.cmd !== 'superelite') {
    import('./seLog-c1b64342.js').then((m) => m.seLog());
  }
}
...
const injectArena = () => { runDefault(import('./arena-2fa2d3e4.js').then(function (n) { return n.a; })); };
const arenaDoJoin = () => { runDefault(import('./arenaDoJoin-56e51771.js')); };
const arenaJoin = () => {
  runDefault(import('./arenaJoin-c161292a.js'));
};
const completedArenas = () => {
  runDefault(import('./completedArenas-2234a40f.js'));
};
const setupMoves = () => {
  runDefault(import('./setup-9f3830fd.js'));
};
const storeMoves = () => { runDefault(import('./store-d4b0b08e.js')); };
const results = () => { runDefault(import('./results-42dfdf2e.js')); };
...
const injectAuctionHouse = () => {
  runDefault(import('./injectAuctionHouse-6255c4b6.js'));
};
const quickCreate = () => {
  runDefault(import('./quickCreate-8cd9b4b1.js'));
};
...
const composingBreakdown = () => { runDefault(import('./breakdown-62e91438.js')); };
const composingCreate = () => { runDefault(import('./composingCreate-2b756b44.js')); };
const injectComposing = () => { runDefault(import('./composing-7db6f996.js')); };
...
const injectAdvisor = () => {
  runDefault(import('./guildAdvisor-06af2507.js'));
};
...
const injectGroups = () => {
  runDefault(import('./groups-08591581.js'));
};
const injectGroupStats = () => {
  runDefault(import('./injectGroupStats-a4bcc392.js'));
};
...
const guildHall = () => { runDefault(import('./hall-c3a82c57.js')); };
...
const injectReportPaint = () => {
  runDefault(import('./guildReport-8e35c18f.js'));
};
const injectGuildAddTagsWidgets = () => {
  runDefault(import('./injectGuildAddTagsWidgets-7e907983.js'));
};
const storeitems = () => {
  runDefault(import('./storeitems-9d9a850a.js'));
};
...
const guildChat = () => { runDefault(import('./guildChat-fbb6bb34.js')); };
const guildLog = () => { runDefault(import('./guildLog-c526b4c9.js')); };
const guildMailbox = () => {
  runDefault(import('./guildMailbox-d57832cf.js'));
};
const injectGuildBank = () => {
  runDefault(import('./injectGuildBank-c0d0449d.js'));
};
const injectGuildRanks = () => {
  runDefault(import('./rank-18743dab.js'));
};
const injectRPUpgrades = () => {
  runDefault(import('./injectRPUpgrades-35d5f4f1.js'));
};
const injectScouttower = () => {
  runDefault(import('./injectScouttower-ef56ddc6.js'));
};
...
function injectWorld() {
  if (isNewMap()) {
    runDefault(import('./newMap-39174dbe.js'));
  }
}

const itemsView = () => { runDefault(import('./itemsView-11daa12f.js')); };
...
const playerLog = () => { runDefault(import('./playerLog-d5370a04.js')); };
const outbox = () => { runDefault(import('./outbox-974b3445.js')); };
...
const newsFsbox = () => { runDefault(import('./newsFsbox-2cbcd48d.js')); };
const newsShoutbox = () => { runDefault(import('./newsShoutbox-ca6d35f4.js')); };
...
const unknownPage = () => { runDefault(import('./unknownPage-0fb104f0.js')); };
...
const injectInventoryManagerNew = () => {
  runDefault(import('./inventory-35a84e79.js'));
};
const injectNewGuildLog = () => {
  runDefault(import('./newGuildLog-4b667113.js'));
};
const injectNotepad = () => {
  runDefault(import('./injectNotepad-a4c3ae0c.js'));
};
const injectSaveSettings = () => { runDefault(import('./load-d708b5d6.js')); };
const reliclist = () => {
  runDefault(import('./reliclist-e527b0cf.js'));
};
const advisor$1 = () => { runDefault(import('./advisor-e144468d.js')); };
const crawler = () => {
  runDefault(import('./crawler-5c0bb863.js'));
};
const newGuildLog5 = () => {
  runDefault(import('./newGuildLog5-4cc9cea9.js'));
};
const whosGotWhat = () => {
  runDefault(import('./whosGotWhat-256eb485.js'));
};
...
const injectQuestTracker = () => {
  runDefault(import('./injectQuestTracker-023b2585.js'));
};
...
const showAllQuestSteps = () => {
  runDefault(import('./showAllQuestSteps-33c91b00.js'));
};
...
const injectScavenging = () => {
  runDefault(import('./scavenging-726ba386.js'));
};
...
const globalQuest = () => { runDefault(import('./globalQuest-3e2f999d.js')); };
const injectTopRated = () => { runDefault(import('./toprated-0c5d2156.js')); };
...
const injectTrade = () => { runDefault(import('./trade-44f43b72.js')); };
...
const craftForge = () => { runDefault(import('./craftForge-8fa807dc.js')); };
const injectBank = () => { runDefault(import('./injectBank-e7dc60e4.js')); };
const injectBazaar = () => { runDefault(import('./bazaar-2427382f.js')); };
const injectFindPlayer = () => {
  runDefault(import('./injectFindPlayer-8f4ca415.js'));
};
const injectMailbox = () => { runDefault(import('./mailbox-91d55f87.js')); };
const injectQuickBuff = () => {
  runDefault(import('./quickBuff-707a15de.js'));
};
const injectTitan = () => { runDefault(import('./injectTitan-d8a3ce3e.js')); };
const injectSettings = () => {
  runDefault(import('./injectSettings-689c8bfc.js'));
};
const ladder = () => { runDefault(import('./ladder-8b88dddb.js')); };
const marketplace = () => { runDefault(import('./marketplace-1d2350e5.js')); };
const points = () => { runDefault(import('./points-4016bdce.js')); };
const superelite = () => { runDefault(import('./superelite-dd336a4c.js')); };
...
//# sourceMappingURL=calfSystem-2b1fed3f.js.map
