...
const allowBack = () => { runDefault(import('./allowBack-36a67a4e.js')); };
const injectBioWidgets = () => { runDefault(import('./bioWidgets-f36dcd6e.js')); };
const injectGuild = () => { runDefault(import('./guild-6574f333.js')); };
const injectProfile = () => { runDefault(import('./profile-a9aa988f.js')); };
const injectProfileDropItems = () => { runDefault(import('./injectProfileDropItems-8bdf8592.js')); };
const injectQuestBookFull = () => { runDefault(import('./injectQuestBookFull-fcdf1972.js')); };
const injectStoreItems = () => { runDefault(import('./injectStoreItems-fbaa853c.js')); };
const inventing = () => { runDefault(import('./inventing-12ea96c1.js')); };
const news = () => { runDefault(import('./news-a3f31b98.js')); };
const viewArchive = () => { runDefault(import('./viewArchive-6b49e6b2.js')); };

const injectBuffLog = (i) => { import('./injectBuffLog-8f8f9cce.js').then((m) => m.default(i)); };
const injectFsBoxContent = (i) => { import('./injectFsBoxContent-cc8a09b9.js').then((m) => m.default(i)); };
const injectMonsterLog = (i) => { import('./monstorLog-3eb3dd88.js').then((m) => m.default(i)); };
const injectNotepadShowLogs = (i) => { import('./combatLog-94f20f10.js').then((m) => m.default(i)); };
const injectOnlinePlayers = (i) => { import('./injectOnlinePlayers-89607399.js').then((m) => m.default(i)); };
const injectRecipeManager = (i) => { import('./recipeMgr-ef1524ec.js').then((m) => m.default(i)); };
const insertQuickExtract = (i) => { import('./quickExtract-2f9bae66.js').then((m) => m.default(i)); };
const insertQuickWear = (i) => { import('./quickWear-385a7613.js').then((m) => m.default(i)); };

const injectAuctionSearch = (i) => { import('./lists-1ddad8c6.js').then((m) => m.injectAuctionSearch(i)); };
const injectQuickLinkManager = (i) => { import('./lists-1ddad8c6.js').then((m) => m.injectQuickLinkManager(i)); };
const injectFindBuffs = (i) => { import('./findBuffs-da3c5789.js').then((m) => m.injectFindBuffs(i)); };
const injectFindOther = (i) => { import('./findBuffs-da3c5789.js').then((m) => m.injectFindOther(i)); };
...
function getLinkConfig(theNav, myNav) {
  ...
  if (linkConfig.some(([, b]) => b)) {
    import('./injectItems-2328dbc1.js')
...
function moveUp(title) {
  import('./moveRHSBoxUpOnRHS-a1f9d9ef.js').then((m) => m.default(title));
}

function moveLeft(title) {
  import('./moveRHSBoxToLHS-672d4aa0.js').then((m) => m.default(title));
}
...
function callAllyEnemy() {
  if (calf.enableAllyOnlineList
    ...
    runDefault(import('./allyEnemy-b44a3c15.js'));
  }
}
...
function callBounties() {
  if (calf.enableWantedList
    ...
    runDefault(import('./activeWantedBounties-efbf5497.js'));
  }
}

function callGuildInfo() {
  if (calf.enableGuildInfoWidgets) {
    runDefault(import('./addGuildInfoWidgets-b468b128.js'));
  }
}

function callAllies() {
  if (calf.enableOnlineAlliesWidgets) {
    runDefault(import('./addOnlineAlliesWidgets-d178a46e.js'));
  }
}

function callTemple() {
  if (calf.enableTempleAlert) {
    runDefault(import('./injectTempleAlert-8929d874.js'));
  }
}

function callUpgrade() {
  if (calf.enableUpgradeAlert) {
    runDefault(import('./injectUpgradeAlert-55307d81.js'));
  }
}

function callComposing() {
  if (calf.enableComposingAlert) {
    runDefault(import('./injectComposeAlert-45b45849.js'));
  }
}

function statbar() {
  if (getValue('statBarLinks')) {
    runDefault(import('./statBar-816d1637.js'));
  }
}

function staminaCalc() {
  if (getValue('staminaCalculator')) {
    runDefault(import('./injectStaminaCalculator-172164b2.js'));
  }
}

function levelCalc() {
  if (getValue('levelUpCalculator')) {
    runDefault(import('./injectLevelupCalculator-331c16e0.js'));
  }
}

function fsBoxLog() {
  if (getValue('fsboxlog')) {
    runDefault(import('./injectFSBoxLog-883a99b2.js'));
  }
}

function expandQb() {
  if (getValue('resizeQuickBuff')) {
    runDefault(import('./interceptQuickBuff-a6bb038c.js'));
  }
}

function joinAll() {
  if (getValue('joinAllLink')) {
    runDefault(import('./injectJoinAllLink-8c117de9.js'));
  }
}

function guildLogHref() {
  if (getValue('useNewGuildLog')) {
    runDefault(import('./changeGuildLogHREF-1a1300bf.js'));
  }
}

function gameStats() {
  if (getValue('addServerNode')) {
    runDefault(import('./injectServerNode-0c2700ba.js'));
  }
}

function scoutTower() {
  if (getValue('addScoutTowerLink')) {
    runDefault(import('./scoutTowerLink-96295228.js'));
  }
}

function guildActivityTracker() {
  if (jQueryPresent() && getValue(defEnableGuildActivityTracker)) {
    runDefault(import('./guildActivity-f030a10d.js'));
  }
}

function seTracker() {
  if (jQueryPresent() && calf.enableSeTracker && calf.cmd !== 'superelite') {
    import('./seLog-f112431d.js').then((m) => m.seLog());
  }
}
...
const injectArena = () => { runDefault(import('./arena-02939610.js').then(function (n) { return n.a; })); };
const arenaDoJoin = () => { runDefault(import('./arenaDoJoin-552f2919.js')); };
const arenaJoin = () => {
  runDefault(import('./arenaJoin-0279faa0.js'));
};
const completedArenas = () => {
  runDefault(import('./completedArenas-44d2f456.js'));
};
const setupMoves = () => {
  runDefault(import('./setup-596b2ed7.js'));
};
const storeMoves = () => { runDefault(import('./store-83197b74.js')); };
const results = () => { runDefault(import('./results-d0a85cbf.js')); };
...
const injectAuctionHouse = () => {
  runDefault(import('./injectAuctionHouse-8567cfe8.js'));
};
const quickCreate = () => {
  runDefault(import('./quickCreate-4bccac06.js'));
};
...
const composingBreakdown = () => { runDefault(import('./breakdown-29d3ce4f.js')); };
const composingCreate = () => { runDefault(import('./composingCreate-eb58c79e.js')); };
const injectComposing = () => { runDefault(import('./composing-a24f8187.js')); };
...
const injectAdvisor = () => {
  runDefault(import('./guildAdvisor-13c3d602.js'));
};
...
const injectGroups = () => {
  runDefault(import('./groups-92682f18.js'));
};
const injectGroupStats = () => {
  runDefault(import('./injectGroupStats-12ea6c01.js'));
};
...
const guildHall = () => { runDefault(import('./hall-2b77f438.js')); };
...
const injectGuildAddTagsWidgets = () => {
  runDefault(import('./injectGuildAddTagsWidgets-880eb880.js'));
};
const injectReportPaint = () => {
  runDefault(import('./guildReport-88a5d4e2.js'));
};
...
const guildChat = () => { runDefault(import('./guildChat-8cba7264.js')); };
const guildLog = () => { runDefault(import('./guildLog-024211dc.js')); };
const guildMailbox = () => {
  runDefault(import('./guildMailbox-eb652ef9.js'));
};
const injectGuildBank = () => {
  runDefault(import('./injectGuildBank-6b62129d.js'));
};
const injectGuildRanks = () => {
  runDefault(import('./rank-c2f5d378.js'));
};
const injectRPUpgrades = () => {
  runDefault(import('./injectRPUpgrades-b84d758a.js'));
};
const injectScouttower = () => {
  runDefault(import('./injectScouttower-1877a0dc.js'));
};
...
function injectWorld() {
  if (isNewMap()) {
    runDefault(import('./newMap-e7290032.js'));
  }
}

const itemsView = () => { runDefault(import('./itemsView-5cc22738.js')); };
...
const playerLog = () => { runDefault(import('./playerLog-8216d32c.js')); };
const outbox = () => { runDefault(import('./outbox-8b18b8aa.js')); };
...
const newsFsbox = () => { runDefault(import('./newsFsbox-1333fc82.js')); };
const newsShoutbox = () => { runDefault(import('./newsShoutbox-92bcb5fe.js')); };
...
const unknownPage = () => { runDefault(import('./unknownPage-10d4423f.js')); };
...
const injectInventoryManagerNew = () => {
  runDefault(import('./inventory-dd384f89.js'));
};
const injectNewGuildLog = () => {
  runDefault(import('./newGuildLog-2819ebe0.js'));
};
const injectNotepad = () => {
  runDefault(import('./injectNotepad-1427f632.js'));
};
const injectSaveSettings = () => { runDefault(import('./load-d37d9fb5.js')); };
const reliclist = () => {
  runDefault(import('./reliclist-a3430833.js'));
};
const advisor$1 = () => { runDefault(import('./advisor-be85b981.js')); };
const crawler = () => {
  runDefault(import('./crawler-efb14d16.js'));
};
const newGuildLog5 = () => {
  runDefault(import('./newGuildLog5-25fb5c2b.js'));
};
const whosGotWhat = () => {
  runDefault(import('./whosGotWhat-df00d063.js'));
};
...
const injectQuestTracker = () => {
  runDefault(import('./injectQuestTracker-1f415a3e.js'));
};
...
const showAllQuestSteps = () => {
  runDefault(import('./showAllQuestSteps-1822e29c.js'));
};
...
const injectScavenging = () => {
  runDefault(import('./scavenging-e58c39cb.js'));
};
...
const globalQuest = () => { runDefault(import('./globalQuest-c61e5e6d.js')); };
const injectTopRated = () => { runDefault(import('./toprated-551b8f76.js')); };
...
const injectTrade = () => { runDefault(import('./trade-b2565b36.js')); };
...
const craftForge = () => { runDefault(import('./craftForge-25f2b72c.js')); };
const injectBank = () => { runDefault(import('./injectBank-b90db45f.js')); };
const injectBazaar = () => { runDefault(import('./bazaar-0a6336bc.js')); };
const injectFindPlayer = () => {
  runDefault(import('./injectFindPlayer-9c954720.js'));
};
const injectMailbox = () => { runDefault(import('./mailbox-2381d380.js')); };
const injectQuickBuff = () => {
  runDefault(import('./quickBuff-d22af77a.js'));
};
const injectTitan = () => { runDefault(import('./injectTitan-847f0dfe.js')); };
const injectSettings = () => {
  runDefault(import('./injectSettings-eeffc36c.js'));
};
const ladder = () => { runDefault(import('./ladder-7b5470f2.js')); };
const marketplace = () => { runDefault(import('./marketplace-e70da48a.js')); };
const points = () => { runDefault(import('./points-3ecfb691.js')); };
const superelite = () => { runDefault(import('./superelite-f4241340.js')); };
...
//# sourceMappingURL=calfSystem-940bc1b5.js.map
