...
const injectBioWidgets = () => { runDefault(import('./bioWidgets-cb58df23.js')); };
const injectGuild = () => { runDefault(import('./guild-de7ae4b1.js')); };
const injectProfile = () => { runDefault(import('./profile-75c69879.js').then(function (n) { return n.p; })); };
const injectProfileDropItems = () => { runDefault(import('./injectProfileDropItems-fc4e654b.js')); };
const injectQuestBookFull = () => { runDefault(import('./injectQuestBookFull-0b16909c.js')); };
const inventing = () => { runDefault(import('./inventing-3e94c7c5.js')); };
const news = () => { runDefault(import('./news-9e281b57.js')); };
const ufsgAllowBack = () => { runDefault(import('./ufsgAllowBack-bf2d9376.js')); };
const viewArchive = () => { runDefault(import('./viewArchive-a8d661ec.js')); };

const injectBuffLog = (i) => { import('./injectBuffLog-8f3c88dc.js').then((m) => m.default(i)); };
const injectFsBoxContent = (i) => { import('./injectFsBoxContent-321f17c3.js').then((m) => m.default(i)); };
const injectMonsterLog = (i) => { import('./monstorLog-53ec1246.js').then((m) => m.default(i)); };
const injectNotepadShowLogs = (i) => { import('./combatLog-f397053c.js').then((m) => m.default(i)); };
const injectOnlinePlayers = (i) => { import('./injectOnlinePlayers-70132efb.js').then((m) => m.default(i)); };
const injectRecipeManager = (i) => { import('./recipeMgr-294425df.js').then((m) => m.default(i)); };
const insertQuickExtract = (i) => { import('./quickExtract-64ec7053.js').then((m) => m.default(i)); };
const insertQuickWear = (i) => { import('./quickWear-99a4efad.js').then((m) => m.default(i)); };

const injectAuctionSearch = (i) => { import('./lists-48d0a804.js').then((m) => m.injectAuctionSearch(i)); };
const injectQuickLinkManager = (i) => { import('./lists-48d0a804.js').then((m) => m.injectQuickLinkManager(i)); };
const injectFindBuffs = (i) => { import('./findBuffs-36d122a7.js').then((m) => m.injectFindBuffs(i)); };
const injectFindOther = (i) => { import('./findBuffs-36d122a7.js').then((m) => m.injectFindOther(i)); };
...
function getLinkConfig(theNav, myNav) {
  ...
  if (linkConfig.some(([, b]) => b)) {
    import('./injectItems-2af0ab7b.js')
...
function moveUp(title) {
  import('./moveRHSBoxUpOnRHS-578462b7.js').then((m) => m.default(title));
}

function moveLeft(title) {
  import('./moveRHSBoxToLHS-f46cb2ab.js').then((m) => m.default(title));
}
...
function callAllyEnemy() {
  if (calf.enableAllyOnlineList
    ...
    runDefault(import('./allyEnemy-bafc3441.js'));
  }
}
...
function callBounties() {
  if (calf.enableWantedList
    ...
    runDefault(import('./activeWantedBounties-4d7f921d.js'));
  }
}

function callGuildInfo() {
  if (calf.enableGuildInfoWidgets) {
    runDefault(import('./addGuildInfoWidgets-d8fef2c8.js'));
  }
}

function callAllies() {
  if (calf.enableOnlineAlliesWidgets) {
    runDefault(import('./addOnlineAlliesWidgets-3d930242.js'));
  }
}

function callTemple() {
  if (calf.enableTempleAlert) {
    runDefault(import('./injectTempleAlert-e3f4f936.js'));
  }
}

function callUpgrade() {
  if (calf.enableUpgradeAlert) {
    runDefault(import('./injectUpgradeAlert-d64f449e.js'));
  }
}

function callComposing() {
  if (calf.enableComposingAlert) {
    runDefault(import('./injectComposeAlert-966ec5ee.js'));
  }
}

function statbar() {
  if (getValue('statBarLinks')) {
    runDefault(import('./statBar-f2a1a72c.js'));
  }
}

function staminaCalc() {
  if (getValue('staminaCalculator')) {
    runDefault(import('./injectStaminaCalculator-8b6513a9.js'));
  }
}

function levelCalc() {
  if (getValue('levelUpCalculator')) {
    runDefault(import('./injectLevelupCalculator-d2eef544.js'));
  }
}

function fsBoxLog() {
  if (getValue('fsboxlog')) {
    runDefault(import('./injectFSBoxLog-ce360593.js'));
  }
}

function expandQb() {
  if (getValue('resizeQuickBuff')) {
    runDefault(import('./interceptQuickBuff-3a2f840f.js'));
  }
}

function joinAll() {
  if (getValue('joinAllLink')) {
    runDefault(import('./injectJoinAllLink-714d4d02.js'));
  }
}

function guildLogHref() {
  if (getValue('useNewGuildLog')) {
    runDefault(import('./changeGuildLogHREF-eba87c23.js'));
  }
}

function gameStats() {
  if (getValue('addServerNode')) {
    runDefault(import('./injectServerNode-fb301d1b.js'));
  }
}

function scoutTower() {
  if (getValue('addScoutTowerLink')) {
    runDefault(import('./scoutTowerLink-30766cc0.js'));
  }
}

function guildActivityTracker() {
  if (jQueryPresent() && getValue(defEnableGuildActivityTracker)) {
    runDefault(import('./guildActivity-9d1ddcf5.js'));
  }
}

function seTracker() {
  if (jQueryPresent() && calf.enableSeTracker && calf.cmd !== 'superelite') {
    import('./seLog-395cad70.js').then((m) => m.seLog());
  }
}
...
const injectArena = () => { runDefault(import('./arena-77f9af2e.js').then(function (n) { return n.a; })); };
const arenaDoJoin = () => { runDefault(import('./arenaDoJoin-f0e40dde.js')); };
const arenaJoin = () => {
  runDefault(import('./arenaJoin-7dea25d5.js'));
};
const completedArenas = () => {
  runDefault(import('./completedArenas-e1073262.js'));
};
const setupMoves = () => {
  runDefault(import('./setup-6e842637.js'));
};
const storeMoves = () => { runDefault(import('./store-f6e52d15.js')); };
const results = () => { runDefault(import('./results-7e4b1435.js')); };
...
const injectAuctionHouse = () => {
  runDefault(import('./injectAuctionHouse-692ef804.js'));
};
const quickCreate = () => {
  runDefault(import('./quickCreate-7785f686.js'));
};
...
const composingBreakdown = () => { runDefault(import('./breakdown-f7ca3e80.js')); };
const composingCreate = () => { runDefault(import('./composingCreate-8f82b717.js')); };
const injectComposing = () => { runDefault(import('./composing-93aa2bfa.js')); };
...
const injectAdvisor = () => {
  runDefault(import('./guildAdvisor-5cb35c7d.js'));
};
...
const injectGroups = () => {
  runDefault(import('./groups-fc7135fa.js'));
};
const injectGroupStats = () => {
  runDefault(import('./injectGroupStats-fe338435.js'));
};
...
const guildHall = () => { runDefault(import('./hall-fb12086d.js')); };
...
const injectReportPaint = () => {
  runDefault(import('./guildReport-620d885c.js'));
};
const injectGuildAddTagsWidgets = () => {
  runDefault(import('./injectGuildAddTagsWidgets-4cee2a19.js'));
};
const storeitems = () => {
  runDefault(import('./storeitems-a3922150.js'));
};
...
const guildChat = () => { runDefault(import('./guildChat-804b6da5.js')); };
const guildLog = () => { runDefault(import('./guildLog-faca8182.js')); };
const guildMailbox = () => {
  runDefault(import('./guildMailbox-5eb13044.js'));
};
const injectGuildBank = () => {
  runDefault(import('./injectGuildBank-e4bfbc09.js'));
};
const injectGuildRanks = () => {
  runDefault(import('./rank-0450933f.js'));
};
const injectRPUpgrades = () => {
  runDefault(import('./injectRPUpgrades-02f15247.js'));
};
const injectScouttower = () => {
  runDefault(import('./injectScouttower-91d458a3.js'));
};
...
function injectWorld() {
  if (isNewMap()) {
    runDefault(import('./newMap-63d74992.js'));
  }
}

const itemsView = () => { runDefault(import('./itemsView-be2758cf.js')); };
...
const playerLog = () => { runDefault(import('./playerLog-4d275635.js')); };
const outbox = () => { runDefault(import('./outbox-d5530c64.js')); };
...
const newsFsbox = () => { runDefault(import('./newsFsbox-e3f3d376.js')); };
const newsShoutbox = () => { runDefault(import('./newsShoutbox-24e2ddf7.js')); };
...
const unknownPage = () => { runDefault(import('./unknownPage-bb4350a6.js')); };
...
const injectInventoryManagerNew = () => {
  runDefault(import('./inventory-d686eac9.js'));
};
const injectNewGuildLog = () => {
  runDefault(import('./newGuildLog-d1a7c29b.js'));
};
const injectNotepad = () => {
  runDefault(import('./injectNotepad-771fe833.js'));
};
const injectSaveSettings = () => { runDefault(import('./load-f277d458.js')); };
const reliclist = () => {
  runDefault(import('./reliclist-ec6aa497.js'));
};
const advisor$1 = () => { runDefault(import('./advisor-a2f3ed59.js')); };
const crawler = () => {
  runDefault(import('./crawler-5ec8a0b3.js'));
};
const newGuildLog5 = () => {
  runDefault(import('./newGuildLog5-ccfd419f.js'));
};
const whosGotWhat = () => {
  runDefault(import('./whosGotWhat-7df07695.js'));
};
...
const injectQuestTracker = () => {
  runDefault(import('./injectQuestTracker-fa5ca9d3.js'));
};
...
const showAllQuestSteps = () => {
  runDefault(import('./showAllQuestSteps-9a413480.js'));
};
...
const injectScavenging = () => {
  runDefault(import('./scavenging-f038f8c0.js'));
};
...
const globalQuest = () => { runDefault(import('./globalQuest-3cc5fd85.js')); };
const injectTopRated = () => { runDefault(import('./toprated-1bb8768f.js')); };
...
const injectTrade = () => { runDefault(import('./trade-5382ae3b.js')); };
...
const craftForge = () => { runDefault(import('./craftForge-56c15d4f.js')); };
const injectBank = () => { runDefault(import('./injectBank-64c94591.js')); };
const injectBazaar = () => { runDefault(import('./bazaar-17a3e657.js')); };
const injectFindPlayer = () => {
  runDefault(import('./injectFindPlayer-d55c3357.js'));
};
const injectMailbox = () => { runDefault(import('./mailbox-347f432e.js')); };
const injectQuickBuff = () => {
  runDefault(import('./quickBuff-26c13531.js'));
};
const injectTitan = () => { runDefault(import('./injectTitan-43f72dc6.js')); };
const injectSettings = () => {
  runDefault(import('./injectSettings-4559b806.js'));
};
const ladder = () => { runDefault(import('./ladder-186175b4.js')); };
const marketplace = () => { runDefault(import('./marketplace-7be287f2.js')); };
const points = () => { runDefault(import('./points-dad9b8ba.js')); };
const superelite = () => { runDefault(import('./superelite-b80595b7.js')); };
...
//# sourceMappingURL=calfSystem-995e3482.js.map
