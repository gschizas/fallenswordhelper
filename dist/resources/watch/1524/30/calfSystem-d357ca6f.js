...
const injectBioWidgets = () => { runDefault(import('./bioWidgets-b75516bd.js')); };
const injectGuild = () => { runDefault(import('./guild-5c430320.js')); };
const injectProfile = () => { runDefault(import('./profile-6746ccd4.js').then(function (n) { return n.p; })); };
const injectProfileDropItems = () => { runDefault(import('./injectProfileDropItems-0af7fd5b.js')); };
const injectQuestBookFull = () => { runDefault(import('./injectQuestBookFull-cb9984f6.js')); };
const inventing = () => { runDefault(import('./inventing-292414cd.js')); };
const news = () => { runDefault(import('./news-795eed7a.js')); };
const ufsgAllowBack = () => { runDefault(import('./ufsgAllowBack-f1262cde.js')); };
const viewArchive = () => { runDefault(import('./viewArchive-22c7d811.js')); };

const injectBuffLog = (i) => { import('./injectBuffLog-477e9dea.js').then((m) => m.default(i)); };
const injectFsBoxContent = (i) => { import('./injectFsBoxContent-0ff835a0.js').then((m) => m.default(i)); };
const injectMonsterLog = (i) => { import('./monstorLog-854440b6.js').then((m) => m.default(i)); };
const injectNotepadShowLogs = (i) => { import('./combatLog-68c9b986.js').then((m) => m.default(i)); };
const injectOnlinePlayers = (i) => { import('./injectOnlinePlayers-dcfad681.js').then((m) => m.default(i)); };
const injectRecipeManager = (i) => { import('./recipeMgr-63b0ab29.js').then((m) => m.default(i)); };
const insertQuickExtract = (i) => { import('./quickExtract-7a175aa7.js').then((m) => m.default(i)); };
const insertQuickWear = (i) => { import('./quickWear-f27e510b.js').then((m) => m.default(i)); };

const injectAuctionSearch = (i) => { import('./lists-5e933c3d.js').then((m) => m.injectAuctionSearch(i)); };
const injectQuickLinkManager = (i) => { import('./lists-5e933c3d.js').then((m) => m.injectQuickLinkManager(i)); };
const injectFindBuffs = (i) => { import('./findBuffs-6ffd10c6.js').then((m) => m.injectFindBuffs(i)); };
const injectFindOther = (i) => { import('./findBuffs-6ffd10c6.js').then((m) => m.injectFindOther(i)); };
...
function getLinkConfig(theNav, myNav) {
  ...
  if (linkConfig.some(([, b]) => b)) {
    import('./injectItems-9b4a3296.js')
...
function moveUp(title) {
  import('./moveRHSBoxUpOnRHS-68296773.js').then((m) => m.default(title));
}

function moveLeft(title) {
  import('./moveRHSBoxToLHS-01c34262.js').then((m) => m.default(title));
}
...
function callAllyEnemy() {
  if (calf.enableAllyOnlineList
    ...
    runDefault(import('./allyEnemy-cc1617b2.js'));
  }
}
...
function callBounties() {
  if (calf.enableWantedList
    ...
    runDefault(import('./activeWantedBounties-b799cf21.js'));
  }
}

function callGuildInfo() {
  if (calf.enableGuildInfoWidgets) {
    runDefault(import('./addGuildInfoWidgets-e1a3d9ee.js'));
  }
}

function callAllies() {
  if (calf.enableOnlineAlliesWidgets) {
    runDefault(import('./addOnlineAlliesWidgets-06750717.js'));
  }
}

function callTemple() {
  if (calf.enableTempleAlert) {
    runDefault(import('./injectTempleAlert-0f127df1.js'));
  }
}

function callUpgrade() {
  if (calf.enableUpgradeAlert) {
    runDefault(import('./injectUpgradeAlert-aeff7ede.js'));
  }
}

function callComposing() {
  if (calf.enableComposingAlert) {
    runDefault(import('./injectComposeAlert-98205d79.js'));
  }
}

function statbar() {
  if (getValue('statBarLinks')) {
    runDefault(import('./statBar-da0a570b.js'));
  }
}

function staminaCalc() {
  if (getValue('staminaCalculator')) {
    runDefault(import('./injectStaminaCalculator-6690b9c7.js'));
  }
}

function levelCalc() {
  if (getValue('levelUpCalculator')) {
    runDefault(import('./injectLevelupCalculator-a4a7c80a.js'));
  }
}

function fsBoxLog() {
  if (getValue('fsboxlog')) {
    runDefault(import('./injectFSBoxLog-a57724ab.js'));
  }
}

function expandQb() {
  if (getValue('resizeQuickBuff')) {
    runDefault(import('./interceptQuickBuff-6c2ef660.js'));
  }
}

function joinAll() {
  if (getValue('joinAllLink')) {
    runDefault(import('./injectJoinAllLink-f954b4f4.js'));
  }
}

function guildLogHref() {
  if (getValue('useNewGuildLog')) {
    runDefault(import('./changeGuildLogHREF-ef6c0df3.js'));
  }
}

function gameStats() {
  if (getValue('addServerNode')) {
    runDefault(import('./injectServerNode-96ce4081.js'));
  }
}

function scoutTower() {
  if (getValue('addScoutTowerLink')) {
    runDefault(import('./scoutTowerLink-e97e39f2.js'));
  }
}

function guildActivityTracker() {
  if (jQueryPresent() && getValue(defEnableGuildActivityTracker)) {
    runDefault(import('./guildActivity-79200c34.js'));
  }
}

function seTracker() {
  if (jQueryPresent() && calf.enableSeTracker && calf.cmd !== 'superelite') {
    import('./seLog-79dd4f2d.js').then((m) => m.seLog());
  }
}
...
const injectArena = () => { runDefault(import('./arena-83e70851.js').then(function (n) { return n.a; })); };
const arenaDoJoin = () => { runDefault(import('./arenaDoJoin-360cf424.js')); };
const arenaJoin = () => {
  runDefault(import('./arenaJoin-bdbf1cb9.js'));
};
const completedArenas = () => {
  runDefault(import('./completedArenas-3008fe25.js'));
};
const setupMoves = () => {
  runDefault(import('./setup-3c9a0da6.js'));
};
const storeMoves = () => { runDefault(import('./store-d3d0c19b.js')); };
const results = () => { runDefault(import('./results-026672a6.js')); };
...
const injectAuctionHouse = () => {
  runDefault(import('./injectAuctionHouse-cda47a31.js'));
};
const quickCreate = () => {
  runDefault(import('./quickCreate-1189c436.js'));
};
...
const composingBreakdown = () => { runDefault(import('./breakdown-dfc0c510.js')); };
const composingCreate = () => { runDefault(import('./composingCreate-4e8c9af8.js')); };
const injectComposing = () => { runDefault(import('./composing-f182c3a0.js')); };
...
const injectAdvisor = () => {
  runDefault(import('./guildAdvisor-7970d1df.js'));
};
...
const injectGroups = () => {
  runDefault(import('./groups-2dc37c55.js'));
};
const injectGroupStats = () => {
  runDefault(import('./injectGroupStats-9b176d25.js'));
};
...
const guildHall = () => { runDefault(import('./hall-0aae2054.js')); };
...
const injectReportPaint = () => {
  runDefault(import('./guildReport-65693cd9.js'));
};
const injectGuildAddTagsWidgets = () => {
  runDefault(import('./injectGuildAddTagsWidgets-af7c7411.js'));
};
const storeitems = () => {
  runDefault(import('./storeitems-48024781.js'));
};
...
const guildChat = () => { runDefault(import('./guildChat-c42c22ea.js')); };
const guildLog = () => { runDefault(import('./guildLog-69f0d877.js')); };
const guildMailbox = () => {
  runDefault(import('./guildMailbox-1a67b8a7.js'));
};
const injectGuildBank = () => {
  runDefault(import('./injectGuildBank-57ea3d92.js'));
};
const injectGuildRanks = () => {
  runDefault(import('./rank-a78ee298.js'));
};
const injectRPUpgrades = () => {
  runDefault(import('./injectRPUpgrades-57f72065.js'));
};
const injectScouttower = () => {
  runDefault(import('./injectScouttower-e10bb3cd.js'));
};
...
function injectWorld() {
  if (isNewMap()) {
    runDefault(import('./newMap-7c7f0231.js'));
  }
}

const itemsView = () => { runDefault(import('./itemsView-9dc12d93.js')); };
...
const playerLog = () => { runDefault(import('./playerLog-9a4a6017.js')); };
const outbox = () => { runDefault(import('./outbox-9a1e64d4.js')); };
...
const newsFsbox = () => { runDefault(import('./newsFsbox-6142c9be.js')); };
const newsShoutbox = () => { runDefault(import('./newsShoutbox-3817583e.js')); };
...
const unknownPage = () => { runDefault(import('./unknownPage-42ef4cf1.js')); };
...
const injectInventoryManagerNew = () => {
  runDefault(import('./inventory-1e26b89d.js'));
};
const injectNewGuildLog = () => {
  runDefault(import('./newGuildLog-6b80b257.js'));
};
const injectNotepad = () => {
  runDefault(import('./injectNotepad-a6e465d4.js'));
};
const injectSaveSettings = () => { runDefault(import('./load-6f30919a.js')); };
const reliclist = () => {
  runDefault(import('./reliclist-5c95bc1e.js'));
};
const advisor$1 = () => { runDefault(import('./advisor-f0332291.js')); };
const crawler = () => {
  runDefault(import('./crawler-9d23479d.js'));
};
const newGuildLog5 = () => {
  runDefault(import('./newGuildLog5-5b54244f.js'));
};
const whosGotWhat = () => {
  runDefault(import('./whosGotWhat-0730f21e.js'));
};
...
const injectQuestTracker = () => {
  runDefault(import('./injectQuestTracker-b5eccfe4.js'));
};
...
const showAllQuestSteps = () => {
  runDefault(import('./showAllQuestSteps-85f7a39b.js'));
};
...
const injectScavenging = () => {
  runDefault(import('./scavenging-175fcef5.js'));
};
...
const globalQuest = () => { runDefault(import('./globalQuest-153b7337.js')); };
const injectTopRated = () => { runDefault(import('./toprated-31bb26bd.js')); };
...
const injectTrade = () => { runDefault(import('./trade-b79b6fca.js')); };
...
const craftForge = () => { runDefault(import('./craftForge-8e3bb1a3.js')); };
const injectBank = () => { runDefault(import('./injectBank-412d0d14.js')); };
const injectBazaar = () => { runDefault(import('./bazaar-710df996.js')); };
const injectFindPlayer = () => {
  runDefault(import('./injectFindPlayer-1ff3d820.js'));
};
const injectMailbox = () => { runDefault(import('./mailbox-c98518a6.js')); };
const injectQuickBuff = () => {
  runDefault(import('./quickBuff-46de8d70.js'));
};
const injectTitan = () => { runDefault(import('./injectTitan-466a99b9.js')); };
const injectSettings = () => {
  runDefault(import('./injectSettings-4e298069.js'));
};
const ladder = () => { runDefault(import('./ladder-fd6e68d8.js')); };
const marketplace = () => { runDefault(import('./marketplace-53e4a92a.js')); };
const points = () => { runDefault(import('./points-27d3c2d6.js')); };
const superelite = () => { runDefault(import('./superelite-964e7075.js')); };
...
//# sourceMappingURL=calfSystem-d357ca6f.js.map
