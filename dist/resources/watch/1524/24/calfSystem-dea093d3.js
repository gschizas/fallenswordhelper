...
const injectBioWidgets = () => { runDefault(import('./bioWidgets-0ace9a69.js')); };
const injectGuild = () => { runDefault(import('./guild-6e4fc6b2.js')); };
const injectProfile = () => { runDefault(import('./profile-219f602b.js').then(function (n) { return n.p; })); };
const injectProfileDropItems = () => { runDefault(import('./injectProfileDropItems-2ea11437.js')); };
const injectQuestBookFull = () => { runDefault(import('./injectQuestBookFull-432cdc02.js')); };
const inventing = () => { runDefault(import('./inventing-1ab2e35a.js')); };
const news = () => { runDefault(import('./news-b399cef0.js')); };
const ufsgAllowBack = () => { runDefault(import('./ufsgAllowBack-fccc253e.js')); };
const viewArchive = () => { runDefault(import('./viewArchive-0b69f487.js')); };

const injectBuffLog = (i) => { import('./injectBuffLog-e94a59e4.js').then((m) => m.default(i)); };
const injectFsBoxContent = (i) => { import('./injectFsBoxContent-9dbc83ee.js').then((m) => m.default(i)); };
const injectMonsterLog = (i) => { import('./monstorLog-18d7ed11.js').then((m) => m.default(i)); };
const injectNotepadShowLogs = (i) => { import('./combatLog-3b610d08.js').then((m) => m.default(i)); };
const injectOnlinePlayers = (i) => { import('./injectOnlinePlayers-4ddce354.js').then((m) => m.default(i)); };
const injectRecipeManager = (i) => { import('./recipeMgr-75354f8f.js').then((m) => m.default(i)); };
const insertQuickExtract = (i) => { import('./quickExtract-af138b05.js').then((m) => m.default(i)); };
const insertQuickWear = (i) => { import('./quickWear-dd9613a3.js').then((m) => m.default(i)); };

const injectAuctionSearch = (i) => { import('./lists-2ba7660c.js').then((m) => m.injectAuctionSearch(i)); };
const injectQuickLinkManager = (i) => { import('./lists-2ba7660c.js').then((m) => m.injectQuickLinkManager(i)); };
const injectFindBuffs = (i) => { import('./findBuffs-c522fa2c.js').then((m) => m.injectFindBuffs(i)); };
const injectFindOther = (i) => { import('./findBuffs-c522fa2c.js').then((m) => m.injectFindOther(i)); };
...
function getLinkConfig(theNav, myNav) {
  ...
  if (linkConfig.some(([, b]) => b)) {
    import('./injectItems-3e6ee1a1.js')
...
function moveUp(title) {
  import('./moveRHSBoxUpOnRHS-3ef8e128.js').then((m) => m.default(title));
}

function moveLeft(title) {
  import('./moveRHSBoxToLHS-7903a600.js').then((m) => m.default(title));
}
...
function callAllyEnemy() {
  if (calf.enableAllyOnlineList
    ...
    runDefault(import('./allyEnemy-4cdd8828.js'));
  }
}
...
function callBounties() {
  if (calf.enableWantedList
    ...
    runDefault(import('./activeWantedBounties-c8e54f15.js'));
  }
}

function callGuildInfo() {
  if (calf.enableGuildInfoWidgets) {
    runDefault(import('./addGuildInfoWidgets-3d6d18ae.js'));
  }
}

function callAllies() {
  if (calf.enableOnlineAlliesWidgets) {
    runDefault(import('./addOnlineAlliesWidgets-e8d41097.js'));
  }
}

function callTemple() {
  if (calf.enableTempleAlert) {
    runDefault(import('./injectTempleAlert-6513b874.js'));
  }
}

function callUpgrade() {
  if (calf.enableUpgradeAlert) {
    runDefault(import('./injectUpgradeAlert-05006c8f.js'));
  }
}

function callComposing() {
  if (calf.enableComposingAlert) {
    runDefault(import('./injectComposeAlert-d17bb9e5.js'));
  }
}

function statbar() {
  if (getValue('statBarLinks')) {
    runDefault(import('./statBar-f71a4845.js'));
  }
}

function staminaCalc() {
  if (getValue('staminaCalculator')) {
    runDefault(import('./injectStaminaCalculator-595fb22b.js'));
  }
}

function levelCalc() {
  if (getValue('levelUpCalculator')) {
    runDefault(import('./injectLevelupCalculator-9d2ccf53.js'));
  }
}

function fsBoxLog() {
  if (getValue('fsboxlog')) {
    runDefault(import('./injectFSBoxLog-4e08a42d.js'));
  }
}

function expandQb() {
  if (getValue('resizeQuickBuff')) {
    runDefault(import('./interceptQuickBuff-41ccaf53.js'));
  }
}

function joinAll() {
  if (getValue('joinAllLink')) {
    runDefault(import('./injectJoinAllLink-554454a2.js'));
  }
}

function guildLogHref() {
  if (getValue('useNewGuildLog')) {
    runDefault(import('./changeGuildLogHREF-dae40501.js'));
  }
}

function gameStats() {
  if (getValue('addServerNode')) {
    runDefault(import('./injectServerNode-b7afb0ff.js'));
  }
}

function scoutTower() {
  if (getValue('addScoutTowerLink')) {
    runDefault(import('./scoutTowerLink-91b9a469.js'));
  }
}

function guildActivityTracker() {
  if (jQueryPresent() && getValue(defEnableGuildActivityTracker)) {
    runDefault(import('./guildActivity-78042127.js'));
  }
}

function seTracker() {
  if (jQueryPresent() && calf.enableSeTracker && calf.cmd !== 'superelite') {
    import('./seLog-036843e5.js').then((m) => m.seLog());
  }
}
...
const injectArena = () => { runDefault(import('./arena-084ecbb0.js').then(function (n) { return n.a; })); };
const arenaDoJoin = () => { runDefault(import('./arenaDoJoin-263f0108.js')); };
const arenaJoin = () => {
  runDefault(import('./arenaJoin-69bbc10d.js'));
};
const completedArenas = () => {
  runDefault(import('./completedArenas-f47e4748.js'));
};
const setupMoves = () => {
  runDefault(import('./setup-b25ac570.js'));
};
const storeMoves = () => { runDefault(import('./store-46dea36c.js')); };
const results = () => { runDefault(import('./results-95fd153a.js')); };
...
const injectAuctionHouse = () => {
  runDefault(import('./injectAuctionHouse-d0176959.js'));
};
const quickCreate = () => {
  runDefault(import('./quickCreate-64848a3c.js'));
};
...
const composingBreakdown = () => { runDefault(import('./breakdown-6d0ceda4.js')); };
const composingCreate = () => { runDefault(import('./composingCreate-e1e04f73.js')); };
const injectComposing = () => { runDefault(import('./composing-7cf20e55.js')); };
...
const injectAdvisor = () => {
  runDefault(import('./guildAdvisor-eabf842f.js'));
};
...
const injectGroups = () => {
  runDefault(import('./groups-70334237.js'));
};
const injectGroupStats = () => {
  runDefault(import('./injectGroupStats-69779723.js'));
};
...
const guildHall = () => { runDefault(import('./hall-bc767681.js')); };
...
const injectReportPaint = () => {
  runDefault(import('./guildReport-ffd72690.js'));
};
const injectGuildAddTagsWidgets = () => {
  runDefault(import('./injectGuildAddTagsWidgets-e76182d4.js'));
};
const storeitems = () => {
  runDefault(import('./storeitems-bb1ae8e3.js'));
};
...
const guildChat = () => { runDefault(import('./guildChat-c63535da.js')); };
const guildLog = () => { runDefault(import('./guildLog-f7f3b00b.js')); };
const guildMailbox = () => {
  runDefault(import('./guildMailbox-f234bb86.js'));
};
const injectGuildBank = () => {
  runDefault(import('./injectGuildBank-5c112725.js'));
};
const injectGuildRanks = () => {
  runDefault(import('./rank-69850d3b.js'));
};
const injectRPUpgrades = () => {
  runDefault(import('./injectRPUpgrades-74234b54.js'));
};
const injectScouttower = () => {
  runDefault(import('./injectScouttower-023147d5.js'));
};
...
function injectWorld() {
  if (isNewMap()) {
    runDefault(import('./newMap-05eb8963.js'));
  }
}

const itemsView = () => { runDefault(import('./itemsView-777eb83c.js')); };
...
const playerLog = () => { runDefault(import('./playerLog-16e80918.js')); };
const outbox = () => { runDefault(import('./outbox-630d2bc4.js')); };
...
const newsFsbox = () => { runDefault(import('./newsFsbox-9f3470ea.js')); };
const newsShoutbox = () => { runDefault(import('./newsShoutbox-21bc5435.js')); };
...
const unknownPage = () => { runDefault(import('./unknownPage-f01a9057.js')); };
...
const injectInventoryManagerNew = () => {
  runDefault(import('./inventory-00699502.js'));
};
const injectNewGuildLog = () => {
  runDefault(import('./newGuildLog-fee53d36.js'));
};
const injectNotepad = () => {
  runDefault(import('./injectNotepad-711f3de1.js'));
};
const injectSaveSettings = () => { runDefault(import('./load-942f0196.js')); };
const reliclist = () => {
  runDefault(import('./reliclist-f5f92608.js'));
};
const advisor$1 = () => { runDefault(import('./advisor-d86513c4.js')); };
const crawler = () => {
  runDefault(import('./crawler-4a085ad4.js'));
};
const newGuildLog5 = () => {
  runDefault(import('./newGuildLog5-1eb007c4.js'));
};
const whosGotWhat = () => {
  runDefault(import('./whosGotWhat-6b3a7335.js'));
};
...
const injectQuestTracker = () => {
  runDefault(import('./injectQuestTracker-8dd142ab.js'));
};
...
const showAllQuestSteps = () => {
  runDefault(import('./showAllQuestSteps-f6fda84e.js'));
};
...
const injectScavenging = () => {
  runDefault(import('./scavenging-52ad5802.js'));
};
...
const globalQuest = () => { runDefault(import('./globalQuest-659dc826.js')); };
const injectTopRated = () => { runDefault(import('./toprated-fc943d2e.js')); };
...
const injectTrade = () => { runDefault(import('./trade-2eb80358.js')); };
...
const craftForge = () => { runDefault(import('./craftForge-e88590ac.js')); };
const injectBank = () => { runDefault(import('./injectBank-c023d30c.js')); };
const injectBazaar = () => { runDefault(import('./bazaar-3e794b03.js')); };
const injectFindPlayer = () => {
  runDefault(import('./injectFindPlayer-9f2458eb.js'));
};
const injectMailbox = () => { runDefault(import('./mailbox-666308b9.js')); };
const injectQuickBuff = () => {
  runDefault(import('./quickBuff-6aa47639.js'));
};
const injectTitan = () => { runDefault(import('./injectTitan-b1855df7.js')); };
const injectSettings = () => {
  runDefault(import('./injectSettings-2ee647d3.js'));
};
const ladder = () => { runDefault(import('./ladder-85aebe38.js')); };
const marketplace = () => { runDefault(import('./marketplace-726d0a12.js')); };
const points = () => { runDefault(import('./points-e79f9bf7.js')); };
const superelite = () => { runDefault(import('./superelite-6383c2e0.js')); };
...
//# sourceMappingURL=calfSystem-dea093d3.js.map
