...
const injectBioWidgets = () => { runDefault(import('./bioWidgets-9d9e152e.js')); };
const injectGuild = () => { runDefault(import('./guild-a438fbac.js')); };
const injectProfile = () => { runDefault(import('./profile-9b59385f.js').then(function (n) { return n.p; })); };
const injectProfileDropItems = () => { runDefault(import('./injectProfileDropItems-0fdad546.js')); };
const injectQuestBookFull = () => { runDefault(import('./injectQuestBookFull-817a4532.js')); };
const inventing = () => { runDefault(import('./inventing-449b7b23.js')); };
const news = () => { runDefault(import('./news-9ab4e38a.js')); };
const ufsgAllowBack = () => { runDefault(import('./ufsgAllowBack-19fe9346.js')); };
const viewArchive = () => { runDefault(import('./viewArchive-e0836bc6.js')); };

const injectBuffLog = (i) => { import('./injectBuffLog-6edf27f6.js').then((m) => m.default(i)); };
const injectFsBoxContent = (i) => { import('./injectFsBoxContent-7185ee19.js').then((m) => m.default(i)); };
const injectMonsterLog = (i) => { import('./monstorLog-5098d4ac.js').then((m) => m.default(i)); };
const injectNotepadShowLogs = (i) => { import('./combatLog-49580cf0.js').then((m) => m.default(i)); };
const injectOnlinePlayers = (i) => { import('./injectOnlinePlayers-61daa95d.js').then((m) => m.default(i)); };
const injectRecipeManager = (i) => { import('./recipeMgr-a196bac6.js').then((m) => m.default(i)); };
const insertQuickExtract = (i) => { import('./quickExtract-e888a490.js').then((m) => m.default(i)); };
const insertQuickWear = (i) => { import('./quickWear-fa975a12.js').then((m) => m.default(i)); };

const injectAuctionSearch = (i) => { import('./lists-ea37e885.js').then((m) => m.injectAuctionSearch(i)); };
const injectQuickLinkManager = (i) => { import('./lists-ea37e885.js').then((m) => m.injectQuickLinkManager(i)); };
const injectFindBuffs = (i) => { import('./findBuffs-b45e9152.js').then((m) => m.injectFindBuffs(i)); };
const injectFindOther = (i) => { import('./findBuffs-b45e9152.js').then((m) => m.injectFindOther(i)); };
...
function getLinkConfig(theNav, myNav) {
  ...
  if (linkConfig.some(([, b]) => b)) {
    import('./injectItems-a2d91cfa.js')
...
function moveUp(title) {
  import('./moveRHSBoxUpOnRHS-f25e6539.js').then((m) => m.default(title));
}

function moveLeft(title) {
  import('./moveRHSBoxToLHS-b791c6a8.js').then((m) => m.default(title));
}
...
function callAllyEnemy() {
  if (calf.enableAllyOnlineList
    ...
    runDefault(import('./allyEnemy-8d5976d8.js'));
  }
}
...
function callBounties() {
  if (calf.enableWantedList
    ...
    runDefault(import('./activeWantedBounties-37e824db.js'));
  }
}

function callGuildInfo() {
  if (calf.enableGuildInfoWidgets) {
    runDefault(import('./addGuildInfoWidgets-6d467076.js'));
  }
}

function callAllies() {
  if (calf.enableOnlineAlliesWidgets) {
    runDefault(import('./addOnlineAlliesWidgets-e402cad4.js'));
  }
}

function callTemple() {
  if (calf.enableTempleAlert) {
    runDefault(import('./injectTempleAlert-1412b449.js'));
  }
}

function callUpgrade() {
  if (calf.enableUpgradeAlert) {
    runDefault(import('./injectUpgradeAlert-d4970528.js'));
  }
}

function callComposing() {
  if (calf.enableComposingAlert) {
    runDefault(import('./injectComposeAlert-c4e3cc95.js'));
  }
}

function statbar() {
  if (getValue('statBarLinks')) {
    runDefault(import('./statBar-82f9aab1.js'));
  }
}

function staminaCalc() {
  if (getValue('staminaCalculator')) {
    runDefault(import('./injectStaminaCalculator-00d595bc.js'));
  }
}

function levelCalc() {
  if (getValue('levelUpCalculator')) {
    runDefault(import('./injectLevelupCalculator-c299648a.js'));
  }
}

function fsBoxLog() {
  if (getValue('fsboxlog')) {
    runDefault(import('./injectFSBoxLog-317d0be5.js'));
  }
}

function expandQb() {
  if (getValue('resizeQuickBuff')) {
    runDefault(import('./interceptQuickBuff-1520c810.js'));
  }
}

function joinAll() {
  if (getValue('joinAllLink')) {
    runDefault(import('./injectJoinAllLink-d07f0692.js'));
  }
}

function guildLogHref() {
  if (getValue('useNewGuildLog')) {
    runDefault(import('./changeGuildLogHREF-316139be.js'));
  }
}

function gameStats() {
  if (getValue('addServerNode')) {
    runDefault(import('./injectServerNode-cd9f6f10.js'));
  }
}

function scoutTower() {
  if (getValue('addScoutTowerLink')) {
    runDefault(import('./scoutTowerLink-96bd1aaa.js'));
  }
}

function guildActivityTracker() {
  if (jQueryPresent() && getValue(defEnableGuildActivityTracker)) {
    runDefault(import('./guildActivity-d414db8d.js'));
  }
}

function seTracker() {
  if (jQueryPresent() && calf.enableSeTracker && calf.cmd !== 'superelite') {
    import('./seLog-247266ff.js').then((m) => m.seLog());
  }
}
...
const injectArena = () => { runDefault(import('./arena-68a71bbe.js').then(function (n) { return n.a; })); };
const arenaDoJoin = () => { runDefault(import('./arenaDoJoin-5cd6c7d2.js')); };
const arenaJoin = () => {
  runDefault(import('./arenaJoin-cd300f21.js'));
};
const completedArenas = () => {
  runDefault(import('./completedArenas-12ba2ead.js'));
};
const setupMoves = () => {
  runDefault(import('./setup-b3b2e222.js'));
};
const storeMoves = () => { runDefault(import('./store-df021c34.js')); };
const results = () => { runDefault(import('./results-0073a009.js')); };
...
const injectAuctionHouse = () => {
  runDefault(import('./injectAuctionHouse-62a00239.js'));
};
const quickCreate = () => {
  runDefault(import('./quickCreate-9f5a73cf.js'));
};
...
const composingBreakdown = () => { runDefault(import('./breakdown-be7ddf65.js')); };
const composingCreate = () => { runDefault(import('./composingCreate-6118b9b8.js')); };
const injectComposing = () => { runDefault(import('./composing-db2231b9.js')); };
...
const injectAdvisor = () => {
  runDefault(import('./guildAdvisor-ef4a6041.js'));
};
...
const injectGroups = () => {
  runDefault(import('./groups-248866da.js'));
};
const injectGroupStats = () => {
  runDefault(import('./injectGroupStats-866aafee.js'));
};
...
const guildHall = () => { runDefault(import('./hall-1b4494a8.js')); };
...
const injectReportPaint = () => {
  runDefault(import('./guildReport-25b757ca.js'));
};
const injectGuildAddTagsWidgets = () => {
  runDefault(import('./injectGuildAddTagsWidgets-dc016056.js'));
};
const storeitems = () => {
  runDefault(import('./storeitems-6378eecf.js'));
};
...
const guildChat = () => { runDefault(import('./guildChat-d6aa5d74.js')); };
const guildLog = () => { runDefault(import('./guildLog-293cc75a.js')); };
const guildMailbox = () => {
  runDefault(import('./guildMailbox-9d4a9bba.js'));
};
const injectGuildBank = () => {
  runDefault(import('./injectGuildBank-d8281bdd.js'));
};
const injectGuildRanks = () => {
  runDefault(import('./rank-4652b9a3.js'));
};
const injectRPUpgrades = () => {
  runDefault(import('./injectRPUpgrades-638ef94f.js'));
};
const injectScouttower = () => {
  runDefault(import('./injectScouttower-f7b457c3.js'));
};
...
function injectWorld() {
  if (isNewMap()) {
    runDefault(import('./newMap-4ca85e32.js'));
  }
}

const itemsView = () => { runDefault(import('./itemsView-92bbecc2.js')); };
...
const playerLog = () => { runDefault(import('./playerLog-560fdaab.js')); };
const outbox = () => { runDefault(import('./outbox-35f7985b.js')); };
...
const newsFsbox = () => { runDefault(import('./newsFsbox-2c836d18.js')); };
const newsShoutbox = () => { runDefault(import('./newsShoutbox-221fb815.js')); };
...
const unknownPage = () => { runDefault(import('./unknownPage-e924adb4.js')); };
...
const injectInventoryManagerNew = () => {
  runDefault(import('./inventory-6817ae90.js'));
};
const injectNewGuildLog = () => {
  runDefault(import('./newGuildLog-76f98714.js'));
};
const injectNotepad = () => {
  runDefault(import('./injectNotepad-f6a0a739.js'));
};
const injectSaveSettings = () => { runDefault(import('./load-9167c6a4.js')); };
const reliclist = () => {
  runDefault(import('./reliclist-2c181098.js'));
};
const advisor$1 = () => { runDefault(import('./advisor-659301a4.js')); };
const crawler = () => {
  runDefault(import('./crawler-0de96dbc.js'));
};
const newGuildLog5 = () => {
  runDefault(import('./newGuildLog5-25a2bc3d.js'));
};
const whosGotWhat = () => {
  runDefault(import('./whosGotWhat-7d071d20.js'));
};
...
const injectQuestTracker = () => {
  runDefault(import('./injectQuestTracker-e84b422f.js'));
};
...
const showAllQuestSteps = () => {
  runDefault(import('./showAllQuestSteps-9bd37c37.js'));
};
...
const injectScavenging = () => {
  runDefault(import('./scavenging-855cf981.js'));
};
...
const globalQuest = () => { runDefault(import('./globalQuest-a20cb38c.js')); };
const injectTopRated = () => { runDefault(import('./toprated-63f3ae57.js')); };
...
const injectTrade = () => { runDefault(import('./trade-9ab81c3d.js')); };
...
const craftForge = () => { runDefault(import('./craftForge-c47f1731.js')); };
const injectBank = () => { runDefault(import('./injectBank-1aa951a6.js')); };
const injectBazaar = () => { runDefault(import('./bazaar-c440eaec.js')); };
const injectFindPlayer = () => {
  runDefault(import('./injectFindPlayer-e39ac0f4.js'));
};
const injectMailbox = () => { runDefault(import('./mailbox-1a3262f2.js')); };
const injectQuickBuff = () => {
  runDefault(import('./quickBuff-b316e480.js'));
};
const injectTitan = () => { runDefault(import('./injectTitan-89ec6880.js')); };
const injectSettings = () => {
  runDefault(import('./injectSettings-098235a3.js'));
};
const ladder = () => { runDefault(import('./ladder-60d4e925.js')); };
const marketplace = () => { runDefault(import('./marketplace-59ae8714.js')); };
const points = () => { runDefault(import('./points-99a90426.js')); };
const superelite = () => { runDefault(import('./superelite-a81ffb4e.js')); };
...
//# sourceMappingURL=calfSystem-b31646eb.js.map
