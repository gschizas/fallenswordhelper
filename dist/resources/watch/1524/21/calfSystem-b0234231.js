...
const injectBioWidgets = () => { runDefault(import('./bioWidgets-43bee25a.js')); };
const injectGuild = () => { runDefault(import('./guild-fd9e919e.js')); };
const injectProfile = () => { runDefault(import('./profile-6f0adbc7.js').then(function (n) { return n.p; })); };
const injectProfileDropItems = () => { runDefault(import('./injectProfileDropItems-ae5876ac.js')); };
const injectQuestBookFull = () => { runDefault(import('./injectQuestBookFull-eaa79dda.js')); };
const inventing = () => { runDefault(import('./inventing-0b8bdd07.js')); };
const news = () => { runDefault(import('./news-33ee375b.js')); };
const ufsgAllowBack = () => { runDefault(import('./ufsgAllowBack-704fd6c3.js')); };
const viewArchive = () => { runDefault(import('./viewArchive-0f9eee5f.js')); };

const injectBuffLog = (i) => { import('./injectBuffLog-bb3cd4f8.js').then((m) => m.default(i)); };
const injectFsBoxContent = (i) => { import('./injectFsBoxContent-e88371ef.js').then((m) => m.default(i)); };
const injectMonsterLog = (i) => { import('./monstorLog-127d3271.js').then((m) => m.default(i)); };
const injectNotepadShowLogs = (i) => { import('./combatLog-603be2f4.js').then((m) => m.default(i)); };
const injectOnlinePlayers = (i) => { import('./injectOnlinePlayers-f4c72944.js').then((m) => m.default(i)); };
const injectRecipeManager = (i) => { import('./recipeMgr-ba4e4f43.js').then((m) => m.default(i)); };
const insertQuickExtract = (i) => { import('./quickExtract-4fd18879.js').then((m) => m.default(i)); };
const insertQuickWear = (i) => { import('./quickWear-1cb8cdd9.js').then((m) => m.default(i)); };

const injectAuctionSearch = (i) => { import('./lists-0e657bed.js').then((m) => m.injectAuctionSearch(i)); };
const injectQuickLinkManager = (i) => { import('./lists-0e657bed.js').then((m) => m.injectQuickLinkManager(i)); };
const injectFindBuffs = (i) => { import('./findBuffs-d8e9f3aa.js').then((m) => m.injectFindBuffs(i)); };
const injectFindOther = (i) => { import('./findBuffs-d8e9f3aa.js').then((m) => m.injectFindOther(i)); };
...
function getLinkConfig(theNav, myNav) {
  ...
  if (linkConfig.some(([, b]) => b)) {
    import('./injectItems-7bd2ed19.js')
...
function moveUp(title) {
  import('./moveRHSBoxUpOnRHS-8b0f1368.js').then((m) => m.default(title));
}

function moveLeft(title) {
  import('./moveRHSBoxToLHS-15bf6c32.js').then((m) => m.default(title));
}
...
function callAllyEnemy() {
  if (calf.enableAllyOnlineList
    ...
    runDefault(import('./allyEnemy-8c926c04.js'));
  }
}
...
function callBounties() {
  if (calf.enableWantedList
    ...
    runDefault(import('./activeWantedBounties-54e0de48.js'));
  }
}

function callGuildInfo() {
  if (calf.enableGuildInfoWidgets) {
    runDefault(import('./addGuildInfoWidgets-4eb1baab.js'));
  }
}

function callAllies() {
  if (calf.enableOnlineAlliesWidgets) {
    runDefault(import('./addOnlineAlliesWidgets-80c3a28d.js'));
  }
}

function callTemple() {
  if (calf.enableTempleAlert) {
    runDefault(import('./injectTempleAlert-18c8bd25.js'));
  }
}

function callUpgrade() {
  if (calf.enableUpgradeAlert) {
    runDefault(import('./injectUpgradeAlert-8c6a0733.js'));
  }
}

function callComposing() {
  if (calf.enableComposingAlert) {
    runDefault(import('./injectComposeAlert-0955822e.js'));
  }
}

function statbar() {
  if (getValue('statBarLinks')) {
    runDefault(import('./statBar-ebf4fe02.js'));
  }
}

function staminaCalc() {
  if (getValue('staminaCalculator')) {
    runDefault(import('./injectStaminaCalculator-98823de8.js'));
  }
}

function levelCalc() {
  if (getValue('levelUpCalculator')) {
    runDefault(import('./injectLevelupCalculator-8b2dc4ab.js'));
  }
}

function fsBoxLog() {
  if (getValue('fsboxlog')) {
    runDefault(import('./injectFSBoxLog-90a4f9e3.js'));
  }
}

function expandQb() {
  if (getValue('resizeQuickBuff')) {
    runDefault(import('./interceptQuickBuff-f7b23358.js'));
  }
}

function joinAll() {
  if (getValue('joinAllLink')) {
    runDefault(import('./injectJoinAllLink-c3d18c57.js'));
  }
}

function guildLogHref() {
  if (getValue('useNewGuildLog')) {
    runDefault(import('./changeGuildLogHREF-c2d6962a.js'));
  }
}

function gameStats() {
  if (getValue('addServerNode')) {
    runDefault(import('./injectServerNode-f1a41a81.js'));
  }
}

function scoutTower() {
  if (getValue('addScoutTowerLink')) {
    runDefault(import('./scoutTowerLink-1420210d.js'));
  }
}

function guildActivityTracker() {
  if (jQueryPresent() && getValue(defEnableGuildActivityTracker)) {
    runDefault(import('./guildActivity-6b46dce1.js'));
  }
}

function seTracker() {
  if (jQueryPresent() && calf.enableSeTracker && calf.cmd !== 'superelite') {
    import('./seLog-3202ce5d.js').then((m) => m.seLog());
  }
}
...
const injectArena = () => { runDefault(import('./arena-df28d283.js').then(function (n) { return n.a; })); };
const arenaDoJoin = () => { runDefault(import('./arenaDoJoin-ff98a590.js')); };
const arenaJoin = () => {
  runDefault(import('./arenaJoin-1f88fee8.js'));
};
const completedArenas = () => {
  runDefault(import('./completedArenas-585511d0.js'));
};
const setupMoves = () => {
  runDefault(import('./setup-5e9e974e.js'));
};
const storeMoves = () => { runDefault(import('./store-bb222e61.js')); };
const results = () => { runDefault(import('./results-16947058.js')); };
...
const injectAuctionHouse = () => {
  runDefault(import('./injectAuctionHouse-8ce2da2e.js'));
};
const quickCreate = () => {
  runDefault(import('./quickCreate-7456714c.js'));
};
...
const composingBreakdown = () => { runDefault(import('./breakdown-e3261b9d.js')); };
const composingCreate = () => { runDefault(import('./composingCreate-6d20f37a.js')); };
const injectComposing = () => { runDefault(import('./composing-738b02f4.js')); };
...
const injectAdvisor = () => {
  runDefault(import('./guildAdvisor-c616df77.js'));
};
...
const injectGroups = () => {
  runDefault(import('./groups-74c95afd.js'));
};
const injectGroupStats = () => {
  runDefault(import('./injectGroupStats-43502ec8.js'));
};
...
const guildHall = () => { runDefault(import('./hall-731cc1e8.js')); };
...
const injectReportPaint = () => {
  runDefault(import('./guildReport-1b662382.js'));
};
const injectGuildAddTagsWidgets = () => {
  runDefault(import('./injectGuildAddTagsWidgets-eb87d5ef.js'));
};
const storeitems = () => {
  runDefault(import('./storeitems-c44ef52a.js'));
};
...
const guildChat = () => { runDefault(import('./guildChat-dde7437c.js')); };
const guildLog = () => { runDefault(import('./guildLog-5ca0c051.js')); };
const guildMailbox = () => {
  runDefault(import('./guildMailbox-513429ca.js'));
};
const injectGuildBank = () => {
  runDefault(import('./injectGuildBank-c40a6406.js'));
};
const injectGuildRanks = () => {
  runDefault(import('./rank-68ce3b74.js'));
};
const injectRPUpgrades = () => {
  runDefault(import('./injectRPUpgrades-13b0e986.js'));
};
const injectScouttower = () => {
  runDefault(import('./injectScouttower-742292ec.js'));
};
...
function injectWorld() {
  if (isNewMap()) {
    runDefault(import('./newMap-c6d4e5c0.js'));
  }
}

const itemsView = () => { runDefault(import('./itemsView-26f5edd2.js')); };
...
const playerLog = () => { runDefault(import('./playerLog-7fec1d44.js')); };
const outbox = () => { runDefault(import('./outbox-488df3b9.js')); };
...
const newsFsbox = () => { runDefault(import('./newsFsbox-f3d90675.js')); };
const newsShoutbox = () => { runDefault(import('./newsShoutbox-01140782.js')); };
...
const unknownPage = () => { runDefault(import('./unknownPage-679b425a.js')); };
...
const injectInventoryManagerNew = () => {
  runDefault(import('./inventory-6731ba7d.js'));
};
const injectNewGuildLog = () => {
  runDefault(import('./newGuildLog-8a5e2e5f.js'));
};
const injectNotepad = () => {
  runDefault(import('./injectNotepad-a591796d.js'));
};
const injectSaveSettings = () => { runDefault(import('./load-9b84415a.js')); };
const reliclist = () => {
  runDefault(import('./reliclist-bcc6b77e.js'));
};
const advisor$1 = () => { runDefault(import('./advisor-d17517f0.js')); };
const crawler = () => {
  runDefault(import('./crawler-69c6bdff.js'));
};
const newGuildLog5 = () => {
  runDefault(import('./newGuildLog5-6c077d68.js'));
};
const whosGotWhat = () => {
  runDefault(import('./whosGotWhat-4289f21b.js'));
};
...
const injectQuestTracker = () => {
  runDefault(import('./injectQuestTracker-cc092a66.js'));
};
...
const showAllQuestSteps = () => {
  runDefault(import('./showAllQuestSteps-51194647.js'));
};
...
const injectScavenging = () => {
  runDefault(import('./scavenging-d9055edf.js'));
};
...
const globalQuest = () => { runDefault(import('./globalQuest-b2b92b61.js')); };
const injectTopRated = () => { runDefault(import('./toprated-ef7f8399.js')); };
...
const injectTrade = () => { runDefault(import('./trade-7ad18394.js')); };
...
const craftForge = () => { runDefault(import('./craftForge-9250e589.js')); };
const injectBank = () => { runDefault(import('./injectBank-0ad656f0.js')); };
const injectBazaar = () => { runDefault(import('./bazaar-d95fb148.js')); };
const injectFindPlayer = () => {
  runDefault(import('./injectFindPlayer-a6ac4a7b.js'));
};
const injectMailbox = () => { runDefault(import('./mailbox-3ad57b1d.js')); };
const injectQuickBuff = () => {
  runDefault(import('./quickBuff-841c4382.js'));
};
const injectTitan = () => { runDefault(import('./injectTitan-6c062f3a.js')); };
const injectSettings = () => {
  runDefault(import('./injectSettings-a6020bcc.js'));
};
const ladder = () => { runDefault(import('./ladder-bc039043.js')); };
const marketplace = () => { runDefault(import('./marketplace-b179a102.js')); };
const points = () => { runDefault(import('./points-ca84a33f.js')); };
const superelite = () => { runDefault(import('./superelite-67171377.js')); };
...
//# sourceMappingURL=calfSystem-b0234231.js.map
