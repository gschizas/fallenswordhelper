...
const allowBack = () => { runDefault(import('./allowBack-acce67b1.js')); };
const injectBioWidgets = () => { runDefault(import('./bioWidgets-2b8d4956.js')); };
const injectGuild = () => { runDefault(import('./guild-00ba9a2d.js')); };
const injectProfile = () => { runDefault(import('./profile-22c6e61f.js')); };
const injectProfileDropItems = () => { runDefault(import('./injectProfileDropItems-d57f5e79.js')); };
const injectQuestBookFull = () => { runDefault(import('./injectQuestBookFull-43bd430d.js')); };
const injectStoreItems = () => { runDefault(import('./injectStoreItems-424554c8.js')); };
const inventing = () => { runDefault(import('./inventing-31562f11.js')); };
const news = () => { runDefault(import('./news-a9389210.js')); };
const viewArchive = () => { runDefault(import('./viewArchive-fb18f81a.js')); };

const injectBuffLog = (i) => { import('./injectBuffLog-189e6eeb.js').then((m) => m.default(i)); };
const injectFsBoxContent = (i) => { import('./injectFsBoxContent-f066ee14.js').then((m) => m.default(i)); };
const injectMonsterLog = (i) => { import('./monstorLog-c6645575.js').then((m) => m.default(i)); };
const injectNotepadShowLogs = (i) => { import('./combatLog-bfc1cf0a.js').then((m) => m.default(i)); };
const injectOnlinePlayers = (i) => { import('./injectOnlinePlayers-56ec0491.js').then((m) => m.default(i)); };
const injectRecipeManager = (i) => { import('./recipeMgr-35a9ee31.js').then((m) => m.default(i)); };
const insertQuickExtract = (i) => { import('./quickExtract-859d9823.js').then((m) => m.default(i)); };
const insertQuickWear = (i) => { import('./quickWear-a107a19b.js').then((m) => m.default(i)); };

const injectAuctionSearch = (i) => { import('./lists-6fcea5ac.js').then((m) => m.injectAuctionSearch(i)); };
const injectQuickLinkManager = (i) => { import('./lists-6fcea5ac.js').then((m) => m.injectQuickLinkManager(i)); };
const injectFindBuffs = (i) => { import('./findBuffs-4deade5d.js').then((m) => m.injectFindBuffs(i)); };
const injectFindOther = (i) => { import('./findBuffs-4deade5d.js').then((m) => m.injectFindOther(i)); };
...
function getLinkConfig(theNav, myNav) {
  ...
  if (linkConfig.some(([, b]) => b)) {
    import('./injectItems-338b6696.js')
...
function moveUp(title) {
  import('./moveRHSBoxUpOnRHS-4bae1be2.js').then((m) => m.default(title));
}

function moveLeft(title) {
  import('./moveRHSBoxToLHS-1d7bcc2e.js').then((m) => m.default(title));
}
...
function callAllyEnemy() {
  if (calf.enableAllyOnlineList
    ...
    runDefault(import('./allyEnemy-b572103a.js'));
  }
}
...
function callBounties() {
  if (calf.enableWantedList
    ...
    runDefault(import('./activeWantedBounties-7f5f9756.js'));
  }
}

function callGuildInfo() {
  if (calf.enableGuildInfoWidgets) {
    runDefault(import('./addGuildInfoWidgets-64c8ac3d.js'));
  }
}

function callAllies() {
  if (calf.enableOnlineAlliesWidgets) {
    runDefault(import('./addOnlineAlliesWidgets-bafa00fb.js'));
  }
}

function callTemple() {
  if (calf.enableTempleAlert) {
    runDefault(import('./injectTempleAlert-e0499487.js'));
  }
}

function callUpgrade() {
  if (calf.enableUpgradeAlert) {
    runDefault(import('./injectUpgradeAlert-b313c78b.js'));
  }
}

function callComposing() {
  if (calf.enableComposingAlert) {
    runDefault(import('./injectComposeAlert-bf2d8fc9.js'));
  }
}

function statbar() {
  if (getValue('statBarLinks')) {
    runDefault(import('./statBar-9cc095e9.js'));
  }
}

function staminaCalc() {
  if (getValue('staminaCalculator')) {
    runDefault(import('./injectStaminaCalculator-5185cc6e.js'));
  }
}

function levelCalc() {
  if (getValue('levelUpCalculator')) {
    runDefault(import('./injectLevelupCalculator-2c508d96.js'));
  }
}

function fsBoxLog() {
  if (getValue('fsboxlog')) {
    runDefault(import('./injectFSBoxLog-f618906c.js'));
  }
}

function expandQb() {
  if (getValue('resizeQuickBuff')) {
    runDefault(import('./interceptQuickBuff-3d8c725c.js'));
  }
}

function joinAll() {
  if (getValue('joinAllLink')) {
    runDefault(import('./injectJoinAllLink-98087b4e.js'));
  }
}

function guildLogHref() {
  if (getValue('useNewGuildLog')) {
    runDefault(import('./changeGuildLogHREF-bec5c755.js'));
  }
}

function gameStats() {
  if (getValue('addServerNode')) {
    runDefault(import('./injectServerNode-8ff9c387.js'));
  }
}

function scoutTower() {
  if (getValue('addScoutTowerLink')) {
    runDefault(import('./scoutTowerLink-f51642c8.js'));
  }
}

function guildActivityTracker() {
  if (jQueryPresent() && getValue(defEnableGuildActivityTracker)) {
    runDefault(import('./guildActivity-f6a2205c.js'));
  }
}

function seTracker() {
  if (jQueryPresent() && calf.enableSeTracker && calf.cmd !== 'superelite') {
    import('./seLog-61c05ebd.js').then((m) => m.seLog());
  }
}
...
const injectArena = () => { runDefault(import('./arena-b9cda89e.js').then(function (n) { return n.a; })); };
const arenaDoJoin = () => { runDefault(import('./arenaDoJoin-ed04a6d0.js')); };
const arenaJoin = () => {
  runDefault(import('./arenaJoin-4ad3297f.js'));
};
const completedArenas = () => {
  runDefault(import('./completedArenas-73209acb.js'));
};
const setupMoves = () => {
  runDefault(import('./setup-1401c136.js'));
};
const storeMoves = () => { runDefault(import('./store-a3d05b34.js')); };
const results = () => { runDefault(import('./results-8cfa15ea.js')); };
...
const injectAuctionHouse = () => {
  runDefault(import('./injectAuctionHouse-4eb01409.js'));
};
const quickCreate = () => {
  runDefault(import('./quickCreate-1e23fc30.js'));
};
...
const composingBreakdown = () => { runDefault(import('./breakdown-c27fa55f.js')); };
const composingCreate = () => { runDefault(import('./composingCreate-b42da73c.js')); };
const injectComposing = () => { runDefault(import('./composing-4de1e5a2.js')); };
...
const injectAdvisor = () => {
  runDefault(import('./guildAdvisor-738618ab.js'));
};
...
const injectGroups = () => {
  runDefault(import('./groups-1e9ecdc5.js'));
};
const injectGroupStats = () => {
  runDefault(import('./injectGroupStats-96c80b6f.js'));
};
...
const guildHall = () => { runDefault(import('./hall-44e56342.js')); };
...
const injectGuildAddTagsWidgets = () => {
  runDefault(import('./injectGuildAddTagsWidgets-971a19ca.js'));
};
const injectReportPaint = () => {
  runDefault(import('./guildReport-e44f3b90.js'));
};
...
const guildChat = () => { runDefault(import('./guildChat-2cce88b4.js')); };
const guildLog = () => { runDefault(import('./guildLog-01c6dc17.js')); };
const guildMailbox = () => {
  runDefault(import('./guildMailbox-35d77063.js'));
};
const injectGuildBank = () => {
  runDefault(import('./injectGuildBank-aa8679f9.js'));
};
const injectGuildRanks = () => {
  runDefault(import('./rank-5ed46b3a.js'));
};
const injectRPUpgrades = () => {
  runDefault(import('./injectRPUpgrades-1eabdefb.js'));
};
const injectScouttower = () => {
  runDefault(import('./injectScouttower-ca92d24b.js'));
};
...
function injectWorld() {
  if (isNewMap()) {
    runDefault(import('./newMap-bb2ac1df.js'));
  }
}

const itemsView = () => { runDefault(import('./itemsView-b4743bb1.js')); };
...
const playerLog = () => { runDefault(import('./playerLog-01d45869.js')); };
const outbox = () => { runDefault(import('./outbox-5babaae5.js')); };
...
const newsFsbox = () => { runDefault(import('./newsFsbox-e99e5223.js')); };
const newsShoutbox = () => { runDefault(import('./newsShoutbox-76cd76fb.js')); };
...
const unknownPage = () => { runDefault(import('./unknownPage-14008ddf.js')); };
...
const injectInventoryManagerNew = () => {
  runDefault(import('./inventory-0dbf5fde.js'));
};
const injectNewGuildLog = () => {
  runDefault(import('./newGuildLog-df5e1474.js'));
};
const injectNotepad = () => {
  runDefault(import('./injectNotepad-1ae1c9ff.js'));
};
const injectSaveSettings = () => { runDefault(import('./load-e4365154.js')); };
const reliclist = () => {
  runDefault(import('./reliclist-928764e3.js'));
};
const advisor$1 = () => { runDefault(import('./advisor-5085505a.js')); };
const crawler = () => {
  runDefault(import('./crawler-15b1567e.js'));
};
const newGuildLog5 = () => {
  runDefault(import('./newGuildLog5-3ed20afc.js'));
};
const whosGotWhat = () => {
  runDefault(import('./whosGotWhat-d16c7db0.js'));
};
...
const injectQuestTracker = () => {
  runDefault(import('./injectQuestTracker-b956fdb5.js'));
};
...
const showAllQuestSteps = () => {
  runDefault(import('./showAllQuestSteps-9f6f7b99.js'));
};
...
const injectScavenging = () => {
  runDefault(import('./scavenging-d9c75410.js'));
};
...
const globalQuest = () => { runDefault(import('./globalQuest-4587018c.js')); };
const injectTopRated = () => { runDefault(import('./toprated-bf2c3afd.js')); };
...
const injectTrade = () => { runDefault(import('./trade-58dd6db8.js')); };
...
const craftForge = () => { runDefault(import('./craftForge-2a3415a7.js')); };
const injectBank = () => { runDefault(import('./injectBank-9095d347.js')); };
const injectBazaar = () => { runDefault(import('./bazaar-7553a826.js')); };
const injectFindPlayer = () => {
  runDefault(import('./injectFindPlayer-9db0fb7f.js'));
};
const injectMailbox = () => { runDefault(import('./mailbox-1e477029.js')); };
const injectQuickBuff = () => {
  runDefault(import('./quickBuff-7cfd57d4.js'));
};
const injectTitan = () => { runDefault(import('./injectTitan-c8822a67.js')); };
const injectSettings = () => {
  runDefault(import('./injectSettings-13a0e3bc.js'));
};
const ladder = () => { runDefault(import('./ladder-20ea2eda.js')); };
const marketplace = () => { runDefault(import('./marketplace-c904891a.js')); };
const points = () => { runDefault(import('./points-2d0430de.js')); };
const superelite = () => { runDefault(import('./superelite-f4321dff.js')); };
