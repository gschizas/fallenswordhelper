...
const allowBack = () => { runDefault(import('./allowBack-991110af.js')); };
const injectBioWidgets = () => { runDefault(import('./bioWidgets-f266bb53.js')); };
const injectGuild = () => { runDefault(import('./guild-ad950967.js')); };
const injectProfile = () => { runDefault(import('./profile-e920ab70.js')); };
const injectProfileDropItems = () => { runDefault(import('./injectProfileDropItems-a8d8604b.js')); };
const injectQuestBookFull = () => { runDefault(import('./injectQuestBookFull-86f2dd02.js')); };
const injectStoreItems = () => { runDefault(import('./injectStoreItems-1acba9a4.js')); };
const inventing = () => { runDefault(import('./inventing-ad795005.js')); };
const news = () => { runDefault(import('./news-65e88915.js')); };
const viewArchive = () => { runDefault(import('./viewArchive-c59e16f7.js')); };

const injectBuffLog = (i) => { import('./injectBuffLog-81c2c010.js').then((m) => m.default(i)); };
const injectFsBoxContent = (i) => { import('./injectFsBoxContent-c80e420a.js').then((m) => m.default(i)); };
const injectMonsterLog = (i) => { import('./monstorLog-7bd68cfb.js').then((m) => m.default(i)); };
const injectNotepadShowLogs = (i) => { import('./combatLog-04ba172d.js').then((m) => m.default(i)); };
const injectOnlinePlayers = (i) => { import('./injectOnlinePlayers-5e3c5f3c.js').then((m) => m.default(i)); };
const injectRecipeManager = (i) => { import('./recipeMgr-bdbb26fb.js').then((m) => m.default(i)); };
const insertQuickExtract = (i) => { import('./quickExtract-0fde4798.js').then((m) => m.default(i)); };
const insertQuickWear = (i) => { import('./quickWear-6ac47209.js').then((m) => m.default(i)); };

const injectAuctionSearch = (i) => { import('./lists-98bb8bc8.js').then((m) => m.injectAuctionSearch(i)); };
const injectQuickLinkManager = (i) => { import('./lists-98bb8bc8.js').then((m) => m.injectQuickLinkManager(i)); };
const injectFindBuffs = (i) => { import('./findBuffs-3a261921.js').then((m) => m.injectFindBuffs(i)); };
const injectFindOther = (i) => { import('./findBuffs-3a261921.js').then((m) => m.injectFindOther(i)); };
...
function getLinkConfig(theNav, myNav) {
  ...
  if (linkConfig.some(([, b]) => b)) {
    import('./injectItems-e567d924.js')
...
function moveUp(title) {
  import('./moveRHSBoxUpOnRHS-5f5b710c.js').then((m) => m.default(title));
}

function moveLeft(title) {
  import('./moveRHSBoxToLHS-4493f436.js').then((m) => m.default(title));
}
...
function callAllyEnemy() {
  if (calf.enableAllyOnlineList
    ...
    runDefault(import('./allyEnemy-39f5c145.js'));
  }
}
...
function callBounties() {
  if (calf.enableWantedList
    ...
    runDefault(import('./activeWantedBounties-299e1a9e.js'));
  }
}

function callGuildInfo() {
  if (calf.enableGuildInfoWidgets) {
    runDefault(import('./addGuildInfoWidgets-3f0e1255.js'));
  }
}

function callAllies() {
  if (calf.enableOnlineAlliesWidgets) {
    runDefault(import('./addOnlineAlliesWidgets-f4643c38.js'));
  }
}

function callTemple() {
  if (calf.enableTempleAlert) {
    runDefault(import('./injectTempleAlert-fceb8e2f.js'));
  }
}

function callUpgrade() {
  if (calf.enableUpgradeAlert) {
    runDefault(import('./injectUpgradeAlert-dc5afa7d.js'));
  }
}

function callComposing() {
  if (calf.enableComposingAlert) {
    runDefault(import('./injectComposeAlert-d1b2a06a.js'));
  }
}

function statbar() {
  if (getValue('statBarLinks')) {
    runDefault(import('./statBar-bfc688ac.js'));
  }
}

function staminaCalc() {
  if (getValue('staminaCalculator')) {
    runDefault(import('./injectStaminaCalculator-596ee982.js'));
  }
}

function levelCalc() {
  if (getValue('levelUpCalculator')) {
    runDefault(import('./injectLevelupCalculator-183fd810.js'));
  }
}

function fsBoxLog() {
  if (getValue('fsboxlog')) {
    runDefault(import('./injectFSBoxLog-83516bff.js'));
  }
}

function expandQb() {
  if (getValue('resizeQuickBuff')) {
    runDefault(import('./interceptQuickBuff-dfc502e1.js'));
  }
}

function joinAll() {
  if (getValue('joinAllLink')) {
    runDefault(import('./injectJoinAllLink-b91869b3.js'));
  }
}

function guildLogHref() {
  if (getValue('useNewGuildLog')) {
    runDefault(import('./changeGuildLogHREF-d98d6e51.js'));
  }
}

function gameStats() {
  if (getValue('addServerNode')) {
    runDefault(import('./injectServerNode-df9ac30f.js'));
  }
}

function scoutTower() {
  if (getValue('addScoutTowerLink')) {
    runDefault(import('./scoutTowerLink-7f028759.js'));
  }
}

function guildActivityTracker() {
  if (jQueryPresent() && getValue(defEnableGuildActivityTracker)) {
    runDefault(import('./guildActivity-4f2489ac.js'));
  }
}

function seTracker() {
  if (jQueryPresent() && calf.enableSeTracker && calf.cmd !== 'superelite') {
    import('./seLog-f79de811.js').then((m) => m.seLog());
  }
}
...
const injectArena = () => { runDefault(import('./arena-55951cf4.js').then(function (n) { return n.a; })); };
const arenaDoJoin = () => { runDefault(import('./arenaDoJoin-03cc9061.js')); };
const arenaJoin = () => {
  runDefault(import('./arenaJoin-7d4211ce.js'));
};
const completedArenas = () => {
  runDefault(import('./completedArenas-345d1426.js'));
};
const setupMoves = () => {
  runDefault(import('./setup-ab8f7824.js'));
};
const storeMoves = () => { runDefault(import('./store-c07c936c.js')); };
const results = () => { runDefault(import('./results-0b308edb.js')); };
...
const injectAuctionHouse = () => {
  runDefault(import('./injectAuctionHouse-8e0adb19.js'));
};
const quickCreate = () => {
  runDefault(import('./quickCreate-bc18a9a7.js'));
};
...
const composingBreakdown = () => { runDefault(import('./breakdown-935ae99a.js')); };
const composingCreate = () => { runDefault(import('./composingCreate-06bc5d64.js')); };
const injectComposing = () => { runDefault(import('./composing-07fd1099.js')); };
...
const injectAdvisor = () => {
  runDefault(import('./guildAdvisor-4eeb6354.js'));
};
...
const injectGroups = () => {
  runDefault(import('./groups-f587a852.js'));
};
const injectGroupStats = () => {
  runDefault(import('./injectGroupStats-1bce46fe.js'));
};
...
const guildHall = () => { runDefault(import('./hall-da48cef5.js')); };
...
const injectGuildAddTagsWidgets = () => {
  runDefault(import('./injectGuildAddTagsWidgets-3d7f2ece.js'));
};
const injectReportPaint = () => {
  runDefault(import('./guildReport-dcf6df82.js'));
};
...
const guildChat = () => { runDefault(import('./guildChat-8339b5e6.js')); };
const guildLog = () => { runDefault(import('./guildLog-d673d08d.js')); };
const guildMailbox = () => {
  runDefault(import('./guildMailbox-c7e5dbc1.js'));
};
const injectGuildBank = () => {
  runDefault(import('./injectGuildBank-47efafae.js'));
};
const injectGuildRanks = () => {
  runDefault(import('./rank-b7bee082.js'));
};
const injectRPUpgrades = () => {
  runDefault(import('./injectRPUpgrades-ed6f8817.js'));
};
const injectScouttower = () => {
  runDefault(import('./injectScouttower-b406e54a.js'));
};
...
function injectWorld() {
  if (isNewMap()) {
    runDefault(import('./newMap-a0e253fa.js'));
  }
}

const itemsView = () => { runDefault(import('./itemsView-3ba45165.js')); };
...
const playerLog = () => { runDefault(import('./playerLog-00a79782.js')); };
const outbox = () => { runDefault(import('./outbox-4fc07470.js')); };
...
const newsFsbox = () => { runDefault(import('./newsFsbox-42d265a0.js')); };
const newsShoutbox = () => { runDefault(import('./newsShoutbox-37b7cf18.js')); };
...
const unknownPage = () => { runDefault(import('./unknownPage-69ade34f.js')); };
...
const injectInventoryManagerNew = () => {
  runDefault(import('./inventory-86ac4383.js'));
};
const injectNewGuildLog = () => {
  runDefault(import('./newGuildLog-060af9b3.js'));
};
const injectNotepad = () => {
  runDefault(import('./injectNotepad-3937aa83.js'));
};
const injectSaveSettings = () => { runDefault(import('./load-f4ee7c40.js')); };
const reliclist = () => {
  runDefault(import('./reliclist-ff3ef567.js'));
};
const advisor$1 = () => { runDefault(import('./advisor-956bc04e.js')); };
const crawler = () => {
  runDefault(import('./crawler-494c56e9.js'));
};
const newGuildLog5 = () => {
  runDefault(import('./newGuildLog5-fc0eb8c0.js'));
};
const whosGotWhat = () => {
  runDefault(import('./whosGotWhat-13d6ec9b.js'));
};
...
const injectQuestTracker = () => {
  runDefault(import('./injectQuestTracker-b73aa544.js'));
};
...
const showAllQuestSteps = () => {
  runDefault(import('./showAllQuestSteps-0931c07f.js'));
};
...
const injectScavenging = () => {
  runDefault(import('./scavenging-680d5604.js'));
};
...
const globalQuest = () => { runDefault(import('./globalQuest-e8244395.js')); };
const injectTopRated = () => { runDefault(import('./toprated-9c49447c.js')); };
...
const injectTrade = () => { runDefault(import('./trade-fb7d202e.js')); };
...
const craftForge = () => { runDefault(import('./craftForge-789df2a2.js')); };
const injectBank = () => { runDefault(import('./injectBank-5844cbda.js')); };
const injectBazaar = () => { runDefault(import('./bazaar-77990bd4.js')); };
const injectFindPlayer = () => {
  runDefault(import('./injectFindPlayer-f1f9fea4.js'));
};
const injectMailbox = () => { runDefault(import('./mailbox-0353e148.js')); };
const injectQuickBuff = () => {
  runDefault(import('./quickBuff-e3ac7a7a.js'));
};
const injectTitan = () => { runDefault(import('./injectTitan-d760f441.js')); };
const injectSettings = () => {
  runDefault(import('./injectSettings-c1ab2c90.js'));
};
const ladder = () => { runDefault(import('./ladder-894a3131.js')); };
const marketplace = () => { runDefault(import('./marketplace-b965f7c2.js')); };
const points = () => { runDefault(import('./points-c1daf04b.js')); };
const superelite = () => { runDefault(import('./superelite-43e9b3bb.js')); };
...
//# sourceMappingURL=calfSystem-6e4b53e3.js.map
