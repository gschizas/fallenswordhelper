...
const injectBioWidgets = () => { runDefault(import('./bioWidgets-489a6be8.js')); };
const injectGuild = () => { runDefault(import('./guild-77d397b1.js')); };
const injectProfile = () => { runDefault(import('./profile-4b0d4987.js').then(function (n) { return n.p; })); };
const injectProfileDropItems = () => { runDefault(import('./injectProfileDropItems-1a9492e4.js')); };
const injectQuestBookFull = () => { runDefault(import('./injectQuestBookFull-1d1b0ada.js')); };
const inventing = () => { runDefault(import('./inventing-da9eac86.js')); };
const news = () => { runDefault(import('./news-b62cd797.js')); };
const ufsgAllowBack = () => { runDefault(import('./ufsgAllowBack-8b639b88.js')); };
const viewArchive = () => { runDefault(import('./viewArchive-b3971457.js')); };

const injectBuffLog = (i) => { import('./injectBuffLog-1abaec8f.js').then((m) => m.default(i)); };
const injectFsBoxContent = (i) => { import('./injectFsBoxContent-b2e8a165.js').then((m) => m.default(i)); };
const injectMonsterLog = (i) => { import('./monstorLog-2eff9ff8.js').then((m) => m.default(i)); };
const injectNotepadShowLogs = (i) => { import('./combatLog-a4cf6f95.js').then((m) => m.default(i)); };
const injectOnlinePlayers = (i) => { import('./injectOnlinePlayers-7081de91.js').then((m) => m.default(i)); };
const injectRecipeManager = (i) => { import('./recipeMgr-46ce3cfb.js').then((m) => m.default(i)); };
const insertQuickExtract = (i) => { import('./quickExtract-155bb3d5.js').then((m) => m.default(i)); };
const insertQuickWear = (i) => { import('./quickWear-2149e1f2.js').then((m) => m.default(i)); };

const injectAuctionSearch = (i) => { import('./lists-4e53447e.js').then((m) => m.injectAuctionSearch(i)); };
const injectQuickLinkManager = (i) => { import('./lists-4e53447e.js').then((m) => m.injectQuickLinkManager(i)); };
const injectFindBuffs = (i) => { import('./findBuffs-023a7555.js').then((m) => m.injectFindBuffs(i)); };
const injectFindOther = (i) => { import('./findBuffs-023a7555.js').then((m) => m.injectFindOther(i)); };
...
function getLinkConfig(theNav, myNav) {
  ...
  if (linkConfig.some(([, b]) => b)) {
    import('./injectItems-2afc0f4f.js')
...
function moveUp(title) {
  import('./moveRHSBoxUpOnRHS-425d500d.js').then((m) => m.default(title));
}

function moveLeft(title) {
  import('./moveRHSBoxToLHS-d73d4b24.js').then((m) => m.default(title));
}
...
function callAllyEnemy() {
  if (calf.enableAllyOnlineList
    ...
    runDefault(import('./allyEnemy-15903f25.js'));
  }
}
...
function callBounties() {
  if (calf.enableWantedList
    ...
    runDefault(import('./activeWantedBounties-3cdcf040.js'));
  }
}

function callGuildInfo() {
  if (calf.enableGuildInfoWidgets) {
    runDefault(import('./addGuildInfoWidgets-e6e99ea4.js'));
  }
}

function callAllies() {
  if (calf.enableOnlineAlliesWidgets) {
    runDefault(import('./addOnlineAlliesWidgets-e5d06f9e.js'));
  }
}

function callTemple() {
  if (calf.enableTempleAlert) {
    runDefault(import('./injectTempleAlert-91aac8c7.js'));
  }
}

function callUpgrade() {
  if (calf.enableUpgradeAlert) {
    runDefault(import('./injectUpgradeAlert-f74ddee8.js'));
  }
}

function callComposing() {
  if (calf.enableComposingAlert) {
    runDefault(import('./injectComposeAlert-2d449a78.js'));
  }
}

function statbar() {
  if (getValue('statBarLinks')) {
    runDefault(import('./statBar-d93205a8.js'));
  }
}

function staminaCalc() {
  if (getValue('staminaCalculator')) {
    runDefault(import('./injectStaminaCalculator-4cac8f76.js'));
  }
}

function levelCalc() {
  if (getValue('levelUpCalculator')) {
    runDefault(import('./injectLevelupCalculator-305d0f45.js'));
  }
}

function fsBoxLog() {
  if (getValue('fsboxlog')) {
    runDefault(import('./injectFSBoxLog-68d02dbb.js'));
  }
}

function expandQb() {
  if (getValue('resizeQuickBuff')) {
    runDefault(import('./interceptQuickBuff-7377bbba.js'));
  }
}

function joinAll() {
  if (getValue('joinAllLink')) {
    runDefault(import('./injectJoinAllLink-fca9fc8e.js'));
  }
}

function guildLogHref() {
  if (getValue('useNewGuildLog')) {
    runDefault(import('./changeGuildLogHREF-51844c3e.js'));
  }
}

function gameStats() {
  if (getValue('addServerNode')) {
    runDefault(import('./injectServerNode-470203f6.js'));
  }
}

function scoutTower() {
  if (getValue('addScoutTowerLink')) {
    runDefault(import('./scoutTowerLink-c724d7d6.js'));
  }
}

function guildActivityTracker() {
  if (jQueryPresent() && getValue(defEnableGuildActivityTracker)) {
    runDefault(import('./guildActivity-d07d99b7.js'));
  }
}

function seTracker() {
  if (jQueryPresent() && calf.enableSeTracker && calf.cmd !== 'superelite') {
    import('./seLog-4986e686.js').then((m) => m.seLog());
  }
}
...
const injectArena = () => { runDefault(import('./arena-17141e4f.js').then(function (n) { return n.a; })); };
const arenaDoJoin = () => { runDefault(import('./arenaDoJoin-559b0505.js')); };
const arenaJoin = () => {
  runDefault(import('./arenaJoin-eb008b5f.js'));
};
const completedArenas = () => {
  runDefault(import('./completedArenas-8793fc88.js'));
};
const setupMoves = () => {
  runDefault(import('./setup-fadbe6df.js'));
};
const storeMoves = () => { runDefault(import('./store-0e64ef0a.js')); };
const results = () => { runDefault(import('./results-2483f91c.js')); };
...
const injectAuctionHouse = () => {
  runDefault(import('./injectAuctionHouse-63b8d856.js'));
};
const quickCreate = () => {
  runDefault(import('./quickCreate-a4042ca3.js'));
};
...
const composingBreakdown = () => { runDefault(import('./breakdown-820f88dc.js')); };
const composingCreate = () => { runDefault(import('./composingCreate-29c05a15.js')); };
const injectComposing = () => { runDefault(import('./composing-141c686e.js')); };
...
const injectAdvisor = () => {
  runDefault(import('./guildAdvisor-51a9b460.js'));
};
...
const injectGroups = () => {
  runDefault(import('./groups-33cae413.js'));
};
const injectGroupStats = () => {
  runDefault(import('./injectGroupStats-87467d1b.js'));
};
...
const guildHall = () => { runDefault(import('./hall-99f4c096.js')); };
...
const injectReportPaint = () => {
  runDefault(import('./guildReport-368c0595.js'));
};
const injectGuildAddTagsWidgets = () => {
  runDefault(import('./injectGuildAddTagsWidgets-f9a4534a.js'));
};
const storeitems = () => {
  runDefault(import('./storeitems-39c0679e.js'));
};
...
const guildChat = () => { runDefault(import('./guildChat-3810c49b.js')); };
const guildLog = () => { runDefault(import('./guildLog-a1cf9ee7.js')); };
const guildMailbox = () => {
  runDefault(import('./guildMailbox-c766e6e6.js'));
};
const injectGuildBank = () => {
  runDefault(import('./injectGuildBank-74fab14e.js'));
};
const injectGuildRanks = () => {
  runDefault(import('./rank-bc25eff7.js'));
};
const injectRPUpgrades = () => {
  runDefault(import('./injectRPUpgrades-136aa9fd.js'));
};
const injectScouttower = () => {
  runDefault(import('./injectScouttower-104d675c.js'));
};
...
function injectWorld() {
  if (isNewMap()) {
    runDefault(import('./newMap-d22dc7af.js'));
  }
}

const itemsView = () => { runDefault(import('./itemsView-55734dc9.js')); };
...
const playerLog = () => { runDefault(import('./playerLog-b6f6213b.js')); };
const outbox = () => { runDefault(import('./outbox-5213209a.js')); };
...
const newsFsbox = () => { runDefault(import('./newsFsbox-31cedae3.js')); };
const newsShoutbox = () => { runDefault(import('./newsShoutbox-43ebc0c2.js')); };
...
const unknownPage = () => { runDefault(import('./unknownPage-fdb4d52a.js')); };
...
const injectInventoryManagerNew = () => {
  runDefault(import('./inventory-139f1efd.js'));
};
const injectNewGuildLog = () => {
  runDefault(import('./newGuildLog-40093ba0.js'));
};
const injectNotepad = () => {
  runDefault(import('./injectNotepad-1891709c.js'));
};
const injectSaveSettings = () => { runDefault(import('./load-2d0edecd.js')); };
const reliclist = () => {
  runDefault(import('./reliclist-76139417.js'));
};
const advisor$1 = () => { runDefault(import('./advisor-3cd7ff14.js')); };
const crawler = () => {
  runDefault(import('./crawler-a71b70c2.js'));
};
const newGuildLog5 = () => {
  runDefault(import('./newGuildLog5-a7173b25.js'));
};
const whosGotWhat = () => {
  runDefault(import('./whosGotWhat-8cf4874c.js'));
};
...
const injectQuestTracker = () => {
  runDefault(import('./injectQuestTracker-85d3be43.js'));
};
...
const showAllQuestSteps = () => {
  runDefault(import('./showAllQuestSteps-234150d4.js'));
};
...
const injectScavenging = () => {
  runDefault(import('./scavenging-a6727a06.js'));
};
...
const globalQuest = () => { runDefault(import('./globalQuest-1d7eefbb.js')); };
const injectTopRated = () => { runDefault(import('./toprated-afd9eb77.js')); };
...
const injectTrade = () => { runDefault(import('./trade-2012f883.js')); };
...
const craftForge = () => { runDefault(import('./craftForge-dad242a3.js')); };
const injectBank = () => { runDefault(import('./injectBank-00cb3304.js')); };
const injectBazaar = () => { runDefault(import('./bazaar-65e9f3b9.js')); };
const injectFindPlayer = () => {
  runDefault(import('./injectFindPlayer-dbe6a356.js'));
};
const injectMailbox = () => { runDefault(import('./mailbox-465aec2e.js')); };
const injectQuickBuff = () => {
  runDefault(import('./quickBuff-5a35c62c.js'));
};
const injectTitan = () => { runDefault(import('./injectTitan-ac078662.js')); };
const injectSettings = () => {
  runDefault(import('./injectSettings-fd45e088.js'));
};
const ladder = () => { runDefault(import('./ladder-56360ebc.js')); };
const marketplace = () => { runDefault(import('./marketplace-7f7c04fe.js')); };
const points = () => { runDefault(import('./points-7887028f.js')); };
const superelite = () => { runDefault(import('./superelite-fc5efa75.js')); };
...
//# sourceMappingURL=calfSystem-c851a12c.js.map
