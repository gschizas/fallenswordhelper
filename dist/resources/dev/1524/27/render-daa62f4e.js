import{t,H as n,av as s,c as e,W as f,b as o,p as a,B as u,l as c,A as r,y as i,e as l}from"./calfSystem-ec5e5725.js"
import{r as p}from"./roundToString-1f3d04fc.js"
import{p as b}from"./playerName-6b140f29.js"
import{t as d}from"./toLowerCase-33399b5a.js"
...
function y(t){return`<tr><td>${t[0]}</td><td>: ${t[1][0]}${t[1][1]}</td></tr>`}function v(t,n){return t[n[1][1]]+=n[1][0],t}function x(){const n=l(j.buffs),s=(e=n.reduce(v,{k:0,fsp:0,stam:0,unknown:0}),m.map(t(g,e)).join(""))
var e
r(`<span class="tip-static" data-tipped="This is an estimated cost based on how the script finds the cost associated with buffs from viewing bio. It can be incorrect, please use with discretion.<br><hr><table border=0>${n.map(y).join("")}</table><b>Total: ${s}</b>">Estimated Cost: <b>${s}</b></span>`,i("buffCost")),j.buffCostTotalText=s}function L(t){const n=c("fshBlue",t)
t.classList.toggle("fshBlue"),t.classList.toggle("fshYellow")
const s=u(t)
n?function(t){const n=C(t)
let s,e
n?(s=function(t){return t[0].includes("k")?"k":t[0].includes("f")?"fsp":"stam"}(n),[e]=n[0].match(/([+-]?[.\d]+)/)):(s="unknown",e="1"),j.buffs[u(t)]=[parseFloat(e),s],j.count+=1}(t):(j.count-=1,delete j.buffs[s]),j.count>0?x():(r("&nbsp;",i("buffCost")),j.buffCostTotalText="")}function N(t){return t.tagName&&"SPAN"!==t.tagName?N(t.parentNode):t}function A(t){if("fshSendBuffMsg"===t.target.id)return void function(t){t.count>0&&$(t)}(j)
const n=N(t.target);(function(t){return t.classList&&c("buffLink",t)})(n)&&L(n)}function F(t,n,s){return t.replace(n,`<span id="fshBuff${s}" class="buffLink fshBlue">${n.replace(/(`~)|(~`)|(\{b\})|(\{\/b\})/g,"")}</span>`)}function M(t){let n=t.replace(/\{b\}/g,"`~").replace(/\{\/b\}/g,"~`")
const s=n.match(/`~([^~]|~(?!`))*~`/g)
...
//# sourceMappingURL=render-daa62f4e.js.map
