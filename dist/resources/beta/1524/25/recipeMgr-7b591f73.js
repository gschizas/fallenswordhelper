import{H as t,b2 as e,A as n,y as i,g as r,v as s,i as a,t as o,B as c,bA as d,ax as p,J as u,m,h as f,u as h,x as l,o as g,p as b}from"./calfSystem-d3aab5a8.js"
import"./toLowerCase-c42114e1.js"
import{s as $,g as v}from"./idb-f33380fa.js"
import"./alpha-4977b995.js"
import"./csvSplit-8c1a6c7f.js"
import{s as x}from"./shouldBeArray-2c799eba.js"
import{d as _}from"./doSortParams-14d7185c.js"
import{a as y}from"./stringSort-2981695e.js"
import{a as N}from"./all-3791b7d5.js"
let j,k=[]
function w(t){return`<div class="rmItem"><img class="tip-dynamic" data-tipped="fetchitem.php?item_id=${t.id}&inv_id=-1&t=2&p=${j}&vcode=${t.verify}" src="${t.img}" height="20px" width="20px"><p>${t.amountPresent}/${t.amountNeeded}</p></div>`}function C(t){return`<div class="rmItem"><img class="tip-dynamic" data-tipped="fetchitem.php?item_id=${t.id}&inv_id=-1&t=2&p=${j}&vcode=${t.verify}" src="${t.img}" height="20px" width="20px"><p>${t.amountPresent}/${t.amountNeeded}</p></div>`}function S(t){return!k.includes(t.name)}function T(t){return`<tr class="rmTr"><td class="rmTd"><a href="${t.link}"><img src="${t.img}" height="30px" width="30px"></a></td><td class="rmTd"><a href="${t.link}">${t.name}</a></td><td class="rmTd">${function(t){return t.items?t.items.map(w).join(""):""}(t)}</td><td class="rmTd">${function(t){return t.components?t.components.map(C).join(""):""}(t)}</td><td class="rmTd">${function(t){return t.target?` <img class="tip-dynamic" data-tipped="fetchitem.php?item_id=${t.target.id}&inv_id=-1&t=2&p=${j}&vcode=${t.target.verify}" src="${t.target.img}" height="30px" width="30px"><br/>`:""}(t)}</td></tr>`}function A(i,r){r&&(t("hideRecipes")&&(k=x("hideRecipeNames")),function(t,i){j=e()
...
return N(a)}function z(t){return/\/folder_on\./.test(t.getAttribute("src"))}function H(t,e){return 0!==e}function J(t){return t.value}function M(t,e,n){return p(`${t}&page=${n}`).then(e)}function D(t,e,n){return function(t){return r("option",u("customselect",i("pCC",t))[0]).filter(H).map(J)}(t).map(o(M,e,n))}function G(t,e,n){const i=s(n),r=function(t){return B(t).find(z).parentNode.href}(i),a=o(q,t,e),c=D(i,r,a)
...
return i&&a(t,`Skipping folder "${n}"  as it has the word "quest" in folder name.<br>`),!i}function Q(t,e){return p(e.parentNode.href).then(t)}function U(t,e,n){const i=o(G,t,e),r=function(t,e,n){return B(s(e)).filter(K).filter(o(O,t)).map(o(Q,n))}(t,n,i)
return r.push(i(n)),N(r)}let V,W
function X(){a(W,"Finished parsing ... formatting ..."),$("fsh_recipeBook",V),A(W,V)}function Y(){V={},V.recipe=[],n("<br>Parsing inventing screen ...<br>",W),h({cmd:"inventing"}).then(o(U,W,V)).then(X)}function Z(t,e){V=e,n('<table class="fshInvFilter"><thead><tr><th width="90%"><b>&nbsp;Recipe Manager</b></th><th width="10%" class="fshBtnBox">[<span id="rfsh" class="fshLink">Refresh</span>]</th></tr></thead></table>',t),W=m(),f(t,W),V?A(W,V):Y()}function tt(t){"rfsh"===t.target.id&&Y(),"sortName"===t.target.id&&function(t){_(t.target),V.recipe.sort(y),A(W,V)}(t)}function et(t){if(l())return
const e=t||b
v("fsh_recipeBook").then(o(Z,e)),g(e,tt)}export default et
//# sourceMappingURL=recipeMgr-7b591f73.js.map
